...
type PersonalProgressDirection = 'increased' | 'decreased' | 'unchanged' | 'insufficient';
interface PersonalProgressMetric {
    key: 'water' | 'protein' | 'activity'; goal: number | null; unit: string;
    loggedDays: number; missingDays: number; achievedDays: number; average: number | null;
    adherencePct: number | null; reliable: boolean;
}
interface PersonalProgressComparison {
    key: PersonalProgressMetric['key']; unit: string; current: PersonalProgressMetric; previous: PersonalProgressMetric;
    reliable: boolean; absoluteDifference: number | null; percentageChange: number | null;
    direction: PersonalProgressDirection; limitation: 'none' | 'current' | 'previous' | 'both';
}
interface PersonalProgressWeek { metrics: PersonalProgressMetric[]; symptomLoggedDays: number; symptomMissingDays: number; symptomEvents: number; }
interface PersonalProgressContext {
    generatedAt: string;
    range: { startDate: string; endDate: string; days: number; previousStartDate: string; previousEndDate: string };
    displayUnits: { water: string; weight: string };
    goals: { water: number | null; protein: number | null; activity: number | null; weight: number | null };
    today: UnknownRecord;
    currentWeek: PersonalProgressWeek; previousWeek: PersonalProgressWeek;
    comparisons: PersonalProgressComparison[]; weight: UnknownRecord; symptoms: UnknownRecord;
    coverage: UnknownRecord; limitations: string[];
}
...
function personalRound(value: number): number { return Math.round(value * 10) / 10; }
function personalDirection(value: number | null): PersonalProgressDirection { return value === null ? 'insufficient' : value > 0 ? 'increased' : value < 0 ? 'decreased' : 'unchanged'; }
function personalWeek(keys: string[]): PersonalProgressWeek {
    ...
    const metrics = specs.map((spec) => {
        ...
        return { key: spec.key, goal: spec.goal, unit: spec.unit, loggedDays: logged.length, missingDays: 7 - logged.length, achievedDays, average, adherencePct: spec.goal === null || !logged.length ? null : Math.round(achievedDays / logged.length * 100), reliable: logged.length >= 4 };
    });
    const symptomDays = days.filter((day) => Number(day.symptomsUpdatedAt) > 0);
    return { metrics, symptomLoggedDays: symptomDays.length, symptomMissingDays: 7 - symptomDays.length, symptomEvents: symptomDays.reduce((sum, day) => sum + personalPositiveSymptoms(day), 0) };
}
function personalCompareMetric(current: PersonalProgressMetric, previous: PersonalProgressMetric): PersonalProgressComparison {
    const reliable = current.reliable && previous.reliable && current.average !== null && previous.average !== null;
    const absoluteDifference = reliable ? personalRound(current.average! - previous.average!) : null;
    const percentageChange = reliable && previous.average !== 0 ? personalRound((absoluteDifference! / previous.average!) * 100) : null;
    const limitation = current.reliable ? (previous.reliable ? 'none' : 'previous') : (previous.reliable ? 'current' : 'both');
    return { key: current.key, unit: current.unit, current, previous, reliable, absoluteDifference, percentageChange, direction: reliable ? personalDirection(absoluteDifference) : 'insufficient', limitation };
}
function personalDailyProgress(current: number | null, goal: number | null, logged: boolean): UnknownRecord {
    const usableGoal = goal !== null && Number.isFinite(goal) && goal > 0;
    const value = logged && current !== null && Number.isFinite(current) ? Math.max(0, current) : null;
    return { current: value, goal: usableGoal ? goal : null, logged, percentage: value !== null && usableGoal ? Math.max(0, Math.round((value / goal!) * 100)) : null, remaining: value !== null && usableGoal ? personalRound(Math.max(0, goal! - value)) : null, reached: value !== null && usableGoal ? value >= goal! : null };
}
function buildPersonalProgressContext(): PersonalProgressContext {
    ...
    const weights = state.weights.filter((w) => w.date <= todayKey && Number.isFinite(Number(w.weight))).sort((a, b) => a.date.localeCompare(b.date));
    const latest = weights.length ? weights[weights.length - 1] : null;
    const currentPeriodWeight = weights.filter((w) => currentKeys.includes(w.date)).at(-1) || null;
    const previousPeriodWeight = weights.filter((w) => previousKeys.includes(w.date)).at(-1) || null;
    ...
    const goals = { water: state.settings.waterGoalConfigured ? displayWater(state.settings.waterGoal) : null, protein: state.settings.proteinGoalConfigured ? state.settings.proteinGoal : null, activity: state.settings.activityGoalConfigured ? state.settings.activityGoalMinutes : null, weight: state.settings.weightGoalConfigured ? displayWeight(state.settings.goalWeight) : null };
    const comparisons = currentWeek.metrics.map((metric) => personalCompareMetric(metric, previousWeek.metrics.find((m) => m.key === metric.key)!));
    const weightDifference = currentPeriodWeight && previousPeriodWeight ? personalRound(displayWeight(currentPeriodWeight.weight)! - displayWeight(previousPeriodWeight.weight)!) : null;
    const symptomReliable = currentWeek.symptomLoggedDays >= 4 && previousWeek.symptomLoggedDays >= 4;
    const symptomDifference = symptomReliable ? currentWeek.symptomEvents - previousWeek.symptomEvents : null;
    const limitations = comparisons.filter((c) => !c.reliable).map((c) => `${c.key}:${c.limitation}`);
    if (!symptomReliable) limitations.push(`symptoms:${currentWeek.symptomLoggedDays < 4 && previousWeek.symptomLoggedDays < 4 ? 'both' : currentWeek.symptomLoggedDays < 4 ? 'current' : 'previous'}`);
    if (!currentPeriodWeight || !previousPeriodWeight) limitations.push(`weight:${!currentPeriodWeight && !previousPeriodWeight ? 'both' : !currentPeriodWeight ? 'current' : 'previous'}`);
    return {
        generatedAt: new Date().toISOString(), range: { startDate: currentKeys[0], endDate: todayKey, days: 7, previousStartDate: previousKeys[0], previousEndDate: previousKeys[6] },
        displayUnits: { water: state.settings.units === 'metric' ? 'mL' : 'oz', weight: state.settings.units === 'metric' ? 'kg' : 'lb' }, goals,
        today: {
            water: personalDailyProgress(today ? displayWater(today.water) : null, goals.water, Boolean(today && (today.waterUpdatedAt || today.waterEvents.length))),
            protein: personalDailyProgress(todayProtein, goals.protein, Boolean(today?.meals.length)),
            activity: personalDailyProgress(today?.activityMinutes ?? null, goals.activity, Boolean(today?.activityUpdatedAt))
        },
        currentWeek, previousWeek, comparisons,
        weight: { latest: latest ? displayWeight(latest.weight) : null, latestDate: latest?.date || '', target: goals.weight, distanceToTarget: latest && goals.weight !== null ? personalRound(displayWeight(latest.weight)! - goals.weight) : null, currentPeriod: currentPeriodWeight ? displayWeight(currentPeriodWeight.weight) : null, currentPeriodDate: currentPeriodWeight?.date || '', previousPeriod: previousPeriodWeight ? displayWeight(previousPeriodWeight.weight) : null, previousPeriodDate: previousPeriodWeight?.date || '', difference: weightDifference, direction: personalDirection(weightDifference), reliable: Boolean(currentPeriodWeight && previousPeriodWeight) },
        symptoms: { currentLoggedDays: currentWeek.symptomLoggedDays, currentMissingDays: currentWeek.symptomMissingDays, currentEvents: currentWeek.symptomEvents, previousLoggedDays: previousWeek.symptomLoggedDays, previousMissingDays: previousWeek.symptomMissingDays, previousEvents: previousWeek.symptomEvents, difference: symptomDifference, direction: symptomReliable ? personalDirection(symptomDifference) : 'insufficient', reliable: symptomReliable },
        coverage: { thresholdDays: 4, currentReliableMetrics: currentWeek.metrics.filter((m) => m.reliable).map((m) => m.key), previousReliableMetrics: previousWeek.metrics.filter((m) => m.reliable).map((m) => m.key), overall: currentWeek.metrics.every((m) => m.reliable) ? 'good' : currentWeek.metrics.some((m) => m.reliable) ? 'partial' : 'limited', comparisonRule: 'A metric comparison is reliable only when both periods have at least 4 of 7 logged days.' }, limitations
    };
}
...
function personalDirectionLabel(direction: PersonalProgressDirection): string { if (direction === 'increased') return uiText('↑ Aumentó', '↑ Increased'); if (direction === 'decreased') return uiText('↓ Disminuyó', '↓ Decreased'); if (direction === 'unchanged') return uiText('→ Sin cambio', '→ No change'); return uiText('Datos insuficientes', 'Insufficient data'); }
function personalRangeLabel(start: string, end: string): string { const opts: Intl.DateTimeFormatOptions = { month: 'short', day: 'numeric' }; return `${new Date(`${start}T12:00:00`).toLocaleDateString(isEnglish() ? 'en-US' : 'es-US', opts)}–${new Date(`${end}T12:00:00`).toLocaleDateString(isEnglish() ? 'en-US' : 'es-US', opts)}`; }
function personalLimitationText(comparison: PersonalProgressComparison): string {
    const c = comparison.current.loggedDays, p = comparison.previous.loggedDays;
    if (comparison.limitation === 'both') return uiText(`Comparación limitada: ambos periodos tienen cobertura insuficiente (${c}/7 actual; ${p}/7 anterior). Se requieren al menos 4 días registrados en cada periodo.`, `Limited comparison: both periods have insufficient recorded-data coverage (${c}/7 current; ${p}/7 previous). At least 4 logged days are required in each period.`);
    if (comparison.limitation === 'current') return uiText(`Comparación limitada: el periodo actual contiene solo ${c} de 7 días con datos. Se requieren al menos 4.`, `Limited comparison: the current period contains only ${c} of 7 days with recorded data. At least 4 are required.`);
    if (comparison.limitation === 'previous') return uiText(`Comparación limitada: el periodo anterior contiene solo ${p} de 7 días con datos. Se requieren al menos 4.`, `Limited comparison: the previous period contains only ${p} of 7 days with recorded data. At least 4 are required.`);
    return '';
}
function renderPersonalProgressView(): void {
    ...
    setText('personal-progress-title', 'Metas y progreso personal', 'Goals & Personal Progress'); setText('personal-progress-subtitle', 'Metas compartidas con Configuración, progreso diario y comparación de periodos.', 'Goals shared with Settings, daily progress, and period comparison.'); setText('personal-save-goals-text', 'Guardar metas', 'Save goals'); setText('personal-goal-help', 'Deja un campo vacío y guarda para quitar esa meta. Estos límites detectan errores de entrada; no son recomendaciones médicas.', 'Leave a field blank and save to remove that goal. These limits catch input errors; they are not medical recommendations.'); setText('personal-activity-today-label', 'Actividad de hoy (minutos)', 'Today activity (minutes)'); setText('personal-save-activity-text', 'Guardar', 'Save'); setText('personal-today-label', 'Hoy', 'Today'); setText('personal-periods-label', 'Periodos comparados', 'Compared periods'); setText('personal-week-label', 'Comparación semanal', 'Weekly comparison'); setText('personal-this-week-label', 'Esta semana', 'This Week'); setText('personal-progress-ai-button-text', 'Explicar mi progreso', 'Explain My Progress'); setText('personal-progress-observations-label', 'Observaciones', 'Observations'); setText('personal-progress-limitations-label', 'Límites de datos', 'Data limitations');
    ...
    const waterInput = document.getElementById('personal-water-goal'), weightInput = document.getElementById('personal-weight-goal'); if (waterInput) { waterInput.min = String(Math.ceil(displayWater(8)!)); waterInput.max = String(Math.floor(displayWater(305)!)); } if (weightInput) { weightInput.min = String(Math.ceil(displayWeight(50)!)); weightInput.max = String(Math.floor(displayWeight(1000)!)); }
    ...
    const today = context.today as Record<string, UnknownRecord>; const activityInput = document.getElementById('personal-activity-today') as unknown as HTMLInputElement | null; if (activityInput && document.activeElement !== activityInput) activityInput.value = today.activity.logged ? String(today.activity.current) : '';
    const todayGrid = document.getElementById('personal-today-grid'); if (todayGrid) { const cards: Array<[string, UnknownRecord, string]> = [['water', today.water, context.displayUnits.water], ['protein', today.protein, 'g'], ['activity', today.activity, 'min']]; const goalCard = (key: string, d: UnknownRecord, unit: string) => { const value = d.current, goal = d.goal, percentage = d.percentage, remaining = d.remaining, reached = d.reached; return `<div class="rounded-xl bg-[#111a2e] border border-[#223455] p-3 space-y-1"><span class="text-[8px] uppercase font-bold text-slate-500">${escapeHtml(personalMetricLabel(key))}</span>${goal === null ? `<p class="text-xs font-black text-amber-300">${escapeHtml(uiText('Configurar meta', 'Set Goal'))}</p>` : `<p class="text-sm font-black text-teal-300">${value === null ? '--' : escapeHtml(String(value))} / ${escapeHtml(String(goal))} ${escapeHtml(unit)}</p><p class="text-xs font-black text-cyan-300">${percentage === null ? '--' : `${percentage}%`}</p><p class="text-[9px] text-slate-400">${value === null ? escapeHtml(uiText('Sin registro hoy', 'No entry today')) : reached ? escapeHtml(uiText('Meta alcanzada', 'Goal reached')) : `${escapeHtml(String(remaining))} ${escapeHtml(unit)} ${escapeHtml(uiText('restantes · Meta en progreso', 'remaining · Goal in progress'))}`}</p>`}</div>`; }; const weightDistance = context.weight.distanceToTarget === null ? uiText('N/D', 'N/A') : `${escapeHtml(String(context.weight.distanceToTarget))} ${escapeHtml(context.displayUnits.weight)}`; todayGrid.innerHTML = cards.map(([key, d, unit]) => goalCard(key, d, unit)).join('') + `<div class="rounded-xl bg-[#111a2e] border border-[#223455] p-3 space-y-1"><span class="text-[8px] uppercase font-bold text-slate-500">${escapeHtml(uiText('Peso', 'Weight'))}</span><p class="text-sm font-black text-teal-300">${context.weight.latest === null ? '--' : `${escapeHtml(String(context.weight.latest))} ${escapeHtml(context.displayUnits.weight)}`}</p><p class="text-[9px] text-slate-400">${context.goals.weight === null ? escapeHtml(uiText('Configurar meta', 'Set Goal')) : `${escapeHtml(uiText('Meta', 'Goal'))}: ${escapeHtml(String(context.goals.weight))} ${escapeHtml(context.displayUnits.weight)} · ${escapeHtml(uiText('Distancia', 'Distance'))}: ${weightDistance}`}</p></div>`; }
    const periods = document.getElementById('personal-periods'); if (periods) periods.innerHTML = `<div><strong class="text-slate-300">${escapeHtml(uiText('Periodo actual', 'Current period'))}</strong><p class="text-teal-300">${escapeHtml(personalRangeLabel(context.range.startDate, context.range.endDate))}</p></div><div><strong class="text-slate-300">${escapeHtml(uiText('Periodo anterior', 'Previous period'))}</strong><p class="text-cyan-300">${escapeHtml(personalRangeLabel(context.range.previousStartDate, context.range.previousEndDate))}</p></div>`;
    const weekGrid = document.getElementById('personal-week-grid'); if (weekGrid) weekGrid.innerHTML = context.comparisons.map((comparison) => { const c = comparison.current, p = comparison.previous; const comparisonLine = comparison.reliable ? `${personalDirectionLabel(comparison.direction)} · ${comparison.absoluteDifference! > 0 ? '+' : ''}${comparison.absoluteDifference} ${comparison.unit}${comparison.percentageChange === null ? ` · ${uiText('cambio % N/D', '% change N/A')}` : ` · ${comparison.percentageChange! > 0 ? '+' : ''}${comparison.percentageChange}%`}` : personalLimitationText(comparison); return `<div class="rounded-xl bg-[#111a2e] border border-[#223455] p-3 space-y-1"><span class="text-[9px] uppercase font-black text-slate-400">${escapeHtml(personalMetricLabel(comparison.key))}</span><p class="text-[9px] text-slate-300">${escapeHtml(uiText('Promedio actual', 'Current average'))}: <strong>${c.average === null ? 'N/D' : `${c.average} ${c.unit}`}</strong> · ${escapeHtml(uiText('anterior', 'previous'))}: <strong>${p.average === null ? 'N/A' : `${p.average} ${p.unit}`}</strong></p><p class="text-[9px] text-slate-400">${escapeHtml(uiText('Cobertura actual', 'Current coverage'))}: ${c.loggedDays}/7 · ${escapeHtml(uiText('anterior', 'previous'))}: ${p.loggedDays}/7</p><p class="text-[9px] text-slate-400">${escapeHtml(uiText('Faltantes', 'Missing'))}: ${c.missingDays} · ${escapeHtml(uiText('Meta alcanzada', 'Goal achieved'))}: ${c.goal === null ? 'N/D' : `${c.achievedDays} ${escapeHtml(uiText('de', 'of'))} ${c.loggedDays}`} · ${escapeHtml(uiText('Adherencia', 'Adherence'))}: ${c.adherencePct === null ? 'N/D' : `${c.adherencePct}%`}</p><p class="text-[9px] ${comparison.reliable ? 'text-teal-300' : 'text-amber-300'} leading-relaxed">${escapeHtml(comparisonLine)}</p></div>`; }).join('');
    const thisWeekRange = document.getElementById('personal-this-week-range'); if (thisWeekRange) thisWeekRange.innerText = personalRangeLabel(context.range.startDate, context.range.endDate);
    const weekSummary = document.getElementById('personal-this-week-grid'); if (weekSummary) { const dailyCards = context.comparisons.map((comparison) => `<div class="rounded-xl bg-[#111a2e] border border-[#223455] p-2.5"><strong class="text-[9px] text-slate-200">${escapeHtml(personalMetricLabel(comparison.key))}</strong><p class="text-xs font-black text-teal-300">${comparison.current.adherencePct === null ? 'N/D' : `${comparison.current.adherencePct}%`}</p><p class="text-[8px] ${comparison.reliable ? 'text-slate-400' : 'text-amber-300'}">${escapeHtml(comparison.reliable ? `${personalDirectionLabel(comparison.direction)}${comparison.percentageChange === null ? '' : ` ${comparison.percentageChange! > 0 ? '+' : ''}${comparison.percentageChange}%`}` : uiText('Comparación limitada por cobertura', 'Comparison limited by coverage'))}</p></div>`).join(''); const w = context.weight, s = context.symptoms; const weightLine = w.reliable ? `${personalDirectionLabel(String(w.direction) as PersonalProgressDirection)} · ${Number(w.difference) > 0 ? '+' : ''}${w.difference} ${context.displayUnits.weight}` : uiText('Datos insuficientes en uno o ambos periodos', 'Insufficient data in one or both periods'); const symptomLine = s.reliable ? `${personalDirectionLabel(String(s.direction) as PersonalProgressDirection)} · ${Number(s.difference) > 0 ? '+' : ''}${s.difference} ${uiText('eventos explícitos', 'explicit events')}` : uiText('Datos explícitos insuficientes para comparar', 'Insufficient explicit data to compare'); const overall = String(context.coverage.overall) === 'good' ? uiText('Buena', 'Good') : String(context.coverage.overall) === 'partial' ? uiText('Parcial', 'Partial') : uiText('Limitada', 'Limited'); weekSummary.innerHTML = dailyCards + `<div class="rounded-xl bg-[#111a2e] border border-[#223455] p-2.5"><strong class="text-[9px] text-slate-200">${escapeHtml(uiText('Peso', 'Weight'))}</strong><p class="text-[9px] text-cyan-300 mt-1">${escapeHtml(weightLine)}</p></div><div class="rounded-xl bg-[#111a2e] border border-[#223455] p-2.5"><strong class="text-[9px] text-slate-200">${escapeHtml(uiText('Síntomas', 'Symptoms'))}</strong><p class="text-[9px] text-cyan-300 mt-1">${escapeHtml(symptomLine)}</p><p class="text-[8px] text-slate-500">${s.currentLoggedDays}/7 ${escapeHtml(uiText('días registrados', 'logged days'))} · ${s.currentMissingDays} ${escapeHtml(uiText('faltantes', 'missing'))}</p></div><div class="rounded-xl bg-[#111a2e] border border-[#223455] p-2.5"><strong class="text-[9px] text-slate-200">${escapeHtml(uiText('Cobertura general', 'Overall coverage'))}</strong><p class="text-xs font-black text-teal-300 mt-1">${escapeHtml(overall)}</p></div>`; }
...
function saveAdaptiveGoals(): void {
    const raw = (id: string) => (document.getElementById(id) as unknown as HTMLInputElement | null)?.value.trim() || '';
    const entries = { water: raw('personal-water-goal'), protein: raw('personal-protein-goal'), activity: raw('personal-activity-goal'), weight: raw('personal-weight-goal') };
    const parsed = Object.fromEntries(Object.entries(entries).map(([key, value]) => [key, value === '' ? null : Number(value)])) as Record<string, number | null>;
    const waterStored = parsed.water === null ? null : storageWater(parsed.water), weightStored = parsed.weight === null ? null : storageWeight(parsed.weight);
    const invalid: string[] = [];
    if (parsed.water !== null && (waterStored === null || !Number.isFinite(waterStored) || waterStored < 8 || waterStored > 305)) invalid.push(uiText(`agua (${Math.ceil(displayWater(8)!)}–${Math.floor(displayWater(305)!)} ${waterUnit()})`, `water (${Math.ceil(displayWater(8)!)}–${Math.floor(displayWater(305)!)} ${waterUnit()})`));
    if (parsed.protein !== null && (!Number.isFinite(parsed.protein) || parsed.protein < 10 || parsed.protein > 500)) invalid.push(uiText('proteína (10–500 g)', 'protein (10–500 g)'));
    if (parsed.activity !== null && (!Number.isFinite(parsed.activity) || parsed.activity < 1 || parsed.activity > 1440)) invalid.push(uiText('actividad (1–1440 min)', 'activity (1–1440 min)'));
    if (parsed.weight !== null && (weightStored === null || !Number.isFinite(weightStored) || weightStored < 50 || weightStored > 1000)) invalid.push(uiText(`peso (${Math.ceil(displayWeight(50)!)}–${Math.floor(displayWeight(1000)!)} ${weightUnit()})`, `weight (${Math.ceil(displayWeight(50)!)}–${Math.floor(displayWeight(1000)!)} ${weightUnit()})`));
    if (invalid.length) { showToast(`${uiText('Valores fuera de los límites de entrada', 'Values outside input limits')}: ${invalid.join(', ')}.`); return; }
    state.settings.waterGoalConfigured = parsed.water !== null; if (waterStored !== null) state.settings.waterGoal = waterStored;
    state.settings.proteinGoalConfigured = parsed.protein !== null; if (parsed.protein !== null) state.settings.proteinGoal = parsed.protein;
    state.settings.activityGoalConfigured = parsed.activity !== null; if (parsed.activity !== null) state.settings.activityGoalMinutes = parsed.activity;
    state.settings.weightGoalConfigured = parsed.weight !== null; if (weightStored !== null) state.settings.goalWeight = weightStored;
    markSettingsChanged(['waterGoal', 'waterGoalConfigured', 'proteinGoal', 'proteinGoalConfigured', 'activityGoalMinutes', 'activityGoalConfigured', 'goalWeight', 'weightGoalConfigured']); persistState({ userMutation: true }); renderPersonalProgressView(); showToast(uiText('Metas guardadas. Los campos vacíos se quitaron.', 'Goals saved. Blank goals were removed.'));
}
