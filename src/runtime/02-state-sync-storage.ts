...
function createDefaultState(devicePrefs: Partial<Pick<RuntimeSettings, 'language' | 'units'>> = {}): RuntimeState {
    return {
        ...
        settings: {
            ...
            weightGoalConfigured: false,
            proteinGoalConfigured: false,
            waterGoalConfigured: false,
            activityGoalConfigured: false,
            pensInStock: 4,
...
function currentMonthKey(date: Date = new Date()) {
    const y = date.getFullYear();
    const m = String(date.getMonth() + 1).padStart(2, '0');
    return `${y}-${m}`;
}
function blankAiModelStats(modelName: string): RuntimeAiModelStats {
...
function sanitizeAiUsageEvents(input: unknown): RuntimeAiUsageEvent[] {
    if (!Array.isArray(input)) return [];
    const allowed = new Set<RuntimeAiUsageEventKind>(['action', 'cache-hit', 'generation-success', 'generation-failure', 'translation']);
    const byId = new Map<string, RuntimeAiUsageEvent>();
    input.forEach((raw) => {
        if (!raw || typeof raw !== 'object' || Array.isArray(raw)) return;
        const src = raw as UnknownRecord;
        const id = String(src.id || '').slice(0, 100);
        const kind = String(src.kind || '') as RuntimeAiUsageEventKind;
        const timestamp = Math.max(0, Math.floor(Number(src.timestamp) || 0));
        if (!id || !allowed.has(kind) || !timestamp) return;
        byId.set(id, { id, kind, timestamp });
    });
    return Array.from(byId.values()).sort((a, b) => a.timestamp - b.timestamp).slice(-600);
}
function sanitizeHealthProfile(input: unknown): RuntimeHealthProfile {
    const src: UnknownRecord = input && typeof input === 'object' && !Array.isArray(input) ? input as UnknownRecord : {};
    const sex: RuntimeHealthProfile['sex'] = typeof src.sex === 'string' && ['female', 'male'].includes(src.sex) ? src.sex as RuntimeHealthProfile['sex'] : '';
    const dob = typeof src.dob === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(src.dob) ? src.dob : '';
    const height = Number(src.heightIn);
    const activity: RuntimeHealthProfile['activity'] = typeof src.activity === 'string' && ['sedentary', 'light', 'moderate', 'active', 'very_active'].includes(src.activity) ? src.activity as RuntimeHealthProfile['activity'] : '';
...
function sanitizeAiModelStats(input: unknown, modelName: string): RuntimeAiModelStats {
    const src: UnknownRecord = input && typeof input === 'object' && !Array.isArray(input) ? input as UnknownRecord : {};
    const n = (key: string) => { const v = Number(src[key]); return Number.isFinite(v) && v >= 0 ? Math.floor(v) : 0; };
    return {
...
function sanitizeAiTelemetry(input: unknown): RuntimeAiTelemetry {
    const out: RuntimeAiTelemetry = {};
    ...
    Object.keys(inputRecord).forEach((month) => {
        ...
        if (!bucket || typeof bucket !== 'object' || Array.isArray(bucket))
            return;
        const bucketRecord = bucket as UnknownRecord;
        const devices: StringMap<RuntimeAiDeviceStats> = {};
        const srcDevices: UnknownRecord = bucketRecord.devices && typeof bucketRecord.devices === 'object' && !Array.isArray(bucketRecord.devices) ? bucketRecord.devices as UnknownRecord : {};
        Object.keys(srcDevices).forEach((deviceId) => {
            const d = srcDevices[deviceId];
            if (!d || typeof d !== 'object' || Array.isArray(d))
                return;
            const dRecord = d as UnknownRecord;
            devices[deviceId] = {
                ...
                userActions: Math.max(0, Math.floor(Number(dRecord.userActions) || 0)),
                cacheHits: Math.max(0, Math.floor(Number(dRecord.cacheHits) || 0)),
                apiCallsAvoided: Math.max(0, Math.floor(Number(dRecord.apiCallsAvoided) || 0)),
                generationSuccesses: Math.max(0, Math.floor(Number(dRecord.generationSuccesses) || 0)),
                generationFailures: Math.max(0, Math.floor(Number(dRecord.generationFailures) || 0)),
                events: sanitizeAiUsageEvents(dRecord.events),
...
function sanitizeAiHistory(input: unknown): RuntimeAiHistoryRecord[] {
    if (!Array.isArray(input)) return [];
    const allowedTypes = new Set<RuntimeAiReportType>(['ask-data', 'weekly-checkin', 'pattern-finder', 'visit-prep', 'progress-comparison', 'personal-progress']);
    const out: RuntimeAiHistoryRecord[] = [];
    input.forEach((raw) => {
        if (!raw || typeof raw !== 'object' || Array.isArray(raw)) return;
        const src = raw as UnknownRecord;
        const id = String(src.id || '').trim();
        const type = String(src.type || '') as RuntimeAiReportType;
        if (!/^[A-Za-z0-9_-]{6,120}$/.test(id) || !allowedTypes.has(type)) return;
        const cleanResult: StringMap<string> = {};
        if (src.result && typeof src.result === 'object' && !Array.isArray(src.result)) {
            Object.entries(src.result as UnknownRecord).forEach(([key, value]) => {
                if (!/^[A-Za-z0-9_-]{1,80}$/.test(key)) return;
                cleanResult[key] = String(value ?? '').slice(0, 16000);
            });
        }
        const snapshot = (src.snapshot && typeof src.snapshot === 'object' && !Array.isArray(src.snapshot))
            ? JSON.parse(JSON.stringify(src.snapshot)) as UnknownRecord
            : {};
        const createdAt = Math.max(0, Math.floor(Number(src.createdAt) || 0));
        const updatedAt = Math.max(createdAt, Math.floor(Number(src.updatedAt) || createdAt));
        const deletedAt = Math.max(0, Math.floor(Number(src.deletedAt) || 0));
        const rangeDaysRaw = Math.floor(Number(src.rangeDays) || 0);
        out.push({
            id,
            type,
            createdAt,
            updatedAt: Math.max(updatedAt, deletedAt),
            deletedAt,
            favorite: Boolean(src.favorite),
            fingerprint: String(src.fingerprint || '').slice(0, 160),
            rangeDays: Math.max(0, Math.min(365, rangeDaysRaw)),
            rangeStart: String(src.rangeStart || '').slice(0, 20),
            rangeEnd: String(src.rangeEnd || '').slice(0, 20),
            requestText: String(src.requestText || '').slice(0, 1200),
            result: cleanResult,
            snapshot,
            appVersion: String(src.appVersion || '').slice(0, 32)
        });
    });
    return out.sort((a, b) => b.updatedAt - a.updatedAt);
}
function mergeAiHistory(localValue: unknown, cloudValue: unknown): RuntimeAiHistoryRecord[] {
    const map = new Map<string, RuntimeAiHistoryRecord>();
    const consider = (record: RuntimeAiHistoryRecord) => {
        const previous = map.get(record.id);
        if (!previous || record.updatedAt > previous.updatedAt ||
            (record.updatedAt === previous.updatedAt && stableStringify(record) > stableStringify(previous))) {
            map.set(record.id, record);
        }
    };
    sanitizeAiHistory(cloudValue).forEach(consider);
    sanitizeAiHistory(localValue).forEach(consider);
    return Array.from(map.values()).sort((a, b) => b.updatedAt - a.updatedAt);
}

function resolveGoalConfigured(settingsInput: UnknownRecord, fieldTimes: UnknownRecord, flag: string, value: string): boolean {
    if (Object.prototype.hasOwnProperty.call(settingsInput, flag) && typeof settingsInput[flag] === 'boolean') return Boolean(settingsInput[flag]);
    return Number(fieldTimes[value]) > 0;
}
...
function ensureV4State() {
    ...
    state.aiHistory = sanitizeAiHistory(state.aiHistory);
    if (!state.settings || typeof state.settings !== 'object')
        state.settings = createDefaultState().settings;
    ensureStateMetadata();
    const legacyFields = state.meta.settingsFieldUpdatedAt || {};
    const settingsRecord = state.settings as unknown as UnknownRecord;
    state.settings.weightGoalConfigured = resolveGoalConfigured(settingsRecord, legacyFields, 'weightGoalConfigured', 'goalWeight');
    state.settings.proteinGoalConfigured = resolveGoalConfigured(settingsRecord, legacyFields, 'proteinGoalConfigured', 'proteinGoal');
    state.settings.waterGoalConfigured = resolveGoalConfigured(settingsRecord, legacyFields, 'waterGoalConfigured', 'waterGoal');
    state.settings.activityGoalConfigured = Boolean(state.settings.activityGoalConfigured);
    state.settings.activityGoalMinutes = Math.max(1, Math.min(1440, Number(state.settings.activityGoalMinutes) || 30));
    ...
    Object.keys(state.daysData || {}).forEach((dateKey) => { const day = state.daysData[dateKey]; if (day && typeof day === 'object' && !Array.isArray(day))
        ensureGranularDayData(dateKey, day); });
    ensureInjectionIds();
}
function mergeAiModelStats(a: unknown, b: unknown, modelName: string): RuntimeAiModelStats {
...
function mergeAiTelemetry(localValue: unknown, cloudValue: unknown): RuntimeAiTelemetry {
    ...
    Object.keys(cloud).forEach((month) => {
        ...
        Object.keys(c.devices || {}).forEach((deviceId) => {
            ...
            out[month].devices[deviceId] = {
                ...
                userActions: Math.max(Number(ld.userActions) || 0, Number(cd.userActions) || 0),
                cacheHits: Math.max(Number(ld.cacheHits) || 0, Number(cd.cacheHits) || 0),
                apiCallsAvoided: Math.max(Number(ld.apiCallsAvoided) || 0, Number(cd.apiCallsAvoided) || 0),
                generationSuccesses: Math.max(Number(ld.generationSuccesses) || 0, Number(cd.generationSuccesses) || 0),
                generationFailures: Math.max(Number(ld.generationFailures) || 0, Number(cd.generationFailures) || 0),
                events: sanitizeAiUsageEvents([...(ld.events || []), ...(cd.events || [])]),
...
function ensureCurrentAiDeviceBucket() {
    ...
    if (!state.aiTelemetry[month].devices[deviceId])
        state.aiTelemetry[month].devices[deviceId] = { primary: blankAiModelStats(AI_PRIMARY_MODEL), fallback: blankAiModelStats(AI_FALLBACK_MODEL), failovers: 0, translations: 0, userActions: 0, cacheHits: 0, apiCallsAvoided: 0, generationSuccesses: 0, generationFailures: 0, events: [], updatedAt: 0 };
    ...
    state.aiTelemetry[month].devices[deviceId].events = sanitizeAiUsageEvents(state.aiTelemetry[month].devices[deviceId].events);
    return { month, deviceId, bucket: state.aiTelemetry[month].devices[deviceId] };
}
function addAiUsageEvent(bucket: RuntimeAiDeviceStats, kind: RuntimeAiUsageEventKind, now: number) {
    const id = `${getAiDeviceId()}_${now}_${Math.random().toString(36).slice(2, 8)}`;
    bucket.events = sanitizeAiUsageEvents([...(bucket.events || []), { id, kind, timestamp: now }]);
}
function recordAiUserAction() {
    const { month, bucket } = ensureCurrentAiDeviceBucket();
    const now = mutationNow();
    bucket.userActions += 1;
    addAiUsageEvent(bucket, 'action', now);
    bucket.updatedAt = state.aiTelemetry[month].updatedAt = now;
}
function recordAiCacheHit() {
    const { month, bucket } = ensureCurrentAiDeviceBucket();
    const now = mutationNow();
    bucket.cacheHits += 1;
    bucket.apiCallsAvoided += 1;
    addAiUsageEvent(bucket, 'cache-hit', now);
    bucket.updatedAt = state.aiTelemetry[month].updatedAt = now;
}
function recordAiGenerationOutcome(success: boolean) {
    const { month, bucket } = ensureCurrentAiDeviceBucket();
    const now = mutationNow();
    if (success) bucket.generationSuccesses += 1;
    else bucket.generationFailures += 1;
    addAiUsageEvent(bucket, success ? 'generation-success' : 'generation-failure', now);
    bucket.updatedAt = state.aiTelemetry[month].updatedAt = now;
}
function recordAiAttempt(role: 'primary' | 'fallback', success: boolean, latencyMs: number = 0) {
    const { month, bucket } = ensureCurrentAiDeviceBucket();
...
function recordAiTranslation() {
    ...
    addAiUsageEvent(bucket, 'translation', now);
    bucket.updatedAt = now;
    state.aiTelemetry[month].updatedAt = now;
}
function aggregateAiMonth(month: string = currentMonthKey()) {
    const t = sanitizeAiTelemetry(state.aiTelemetry), bucket = t[month] || { devices: {} };
    const result = { primary: blankAiModelStats(AI_PRIMARY_MODEL), fallback: blankAiModelStats(AI_FALLBACK_MODEL), failovers: 0, translations: 0, userActions: 0, cacheHits: 0, apiCallsAvoided: 0, generationSuccesses: 0, generationFailures: 0 };
    (['primary', 'fallback'] as const).forEach((role) => {
        let latest: RuntimeAiModelStats = blankAiModelStats(role === 'primary' ? AI_PRIMARY_MODEL : AI_FALLBACK_MODEL);
    ...
    Object.values(bucket.devices || {}).forEach((d) => {
        result.failovers += Math.max(0, Number(d.failovers) || 0);
        result.translations += Math.max(0, Number(d.translations) || 0);
        result.userActions += Math.max(0, Number(d.userActions) || 0);
        result.cacheHits += Math.max(0, Number(d.cacheHits) || 0);
        result.apiCallsAvoided += Math.max(0, Number(d.apiCallsAvoided) || 0);
        result.generationSuccesses += Math.max(0, Number(d.generationSuccesses) || 0);
        result.generationFailures += Math.max(0, Number(d.generationFailures) || 0);
    });
    return result;
}
function aggregateAiLast30Days() {
    const cutoff = Date.now() - (30 * 24 * 60 * 60 * 1000);
    const counts = { actions: 0, cacheHits: 0, successes: 0, failures: 0, translations: 0 };
    const seen = new Set<string>();
    Object.values(sanitizeAiTelemetry(state.aiTelemetry)).forEach((month) => Object.values(month.devices || {}).forEach((device) => {
        sanitizeAiUsageEvents(device.events).forEach((event) => {
            if (event.timestamp < cutoff || seen.has(event.id)) return;
            seen.add(event.id);
            if (event.kind === 'action') counts.actions += 1;
            if (event.kind === 'cache-hit') counts.cacheHits += 1;
            if (event.kind === 'generation-success') counts.successes += 1;
            if (event.kind === 'generation-failure') counts.failures += 1;
            if (event.kind === 'translation') counts.translations += 1;
        });
    }));
    return counts;
}
function formatAiLast(ts: unknown) {
    const n = Number(ts);
    if (!n)
        return '--';
...
function renderAiStatus() {
    ...
    const last30 = aggregateAiLast30Days();
    ...
    const apiCalls = agg.primary.attempts + agg.fallback.attempts;
    const modelAttempts = Math.max(1, apiCalls);
    const efficiency = agg.userActions ? Math.round((agg.cacheHits / agg.userActions) * 100) : 0;
    const setText = (id: string, value: string) => { const el = document.getElementById(id); if (el) el.innerText = value; };
    setText('ai-user-actions', String(agg.userActions));
    setText('ai-api-calls', String(apiCalls));
    setText('ai-cache-hits', String(agg.cacheHits));
    setText('ai-calls-avoided', String(agg.apiCallsAvoided));
    setText('ai-cache-efficiency', `${efficiency}%`);
    setText('ai-generation-outcomes', isEnglish() ? `${agg.generationSuccesses} successes · ${agg.generationFailures} failures` : `${agg.generationSuccesses} éxitos · ${agg.generationFailures} fallos`);
    setText('ai-model-share', isEnglish() ? `Primary ${Math.round((agg.primary.attempts / modelAttempts) * 100)}% · Fallback ${Math.round((agg.fallback.attempts / modelAttempts) * 100)}%` : `Principal ${Math.round((agg.primary.attempts / modelAttempts) * 100)}% · Respaldo ${Math.round((agg.fallback.attempts / modelAttempts) * 100)}%`);
    setText('ai-last-30-days', isEnglish()
        ? `Last 30 days: ${last30.actions} actions · ${last30.cacheHits} cache hits · ${last30.successes} generations · ${last30.failures} failures · ${last30.translations} translations.`
        : `Últimos 30 días: ${last30.actions} acciones · ${last30.cacheHits} desde caché · ${last30.successes} generaciones · ${last30.failures} fallos · ${last30.translations} traducciones.`);
    const renderRole = (role: 'primary' | 'fallback', st: RuntimeAiModelStats) => {
...
function ensureStateMetadata() {
    ...
    Object.keys(state.meta.settingsFieldUpdatedAt).forEach((k) => { const n = Number(state.meta.settingsFieldUpdatedAt[k]); if (!Number.isFinite(n) || n < 0)
...
function markSettingsChanged(fields: string[] | null = null) {
    ensureStateMetadata();
    const ts = mutationNow();
    state.meta.settingsUpdatedAt = ts;
    const list = Array.isArray(fields) && fields.length ? fields : cloudSettingKeys();
    list.forEach((k) => { if (k && !['language', 'units'].includes(k))
        state.meta.settingsFieldUpdatedAt[k] = ts; });
    return ts;
}
function getInjectionMedicationKey(injection: RuntimeInjection) {
    const key = injection && injection.medication;
    return key && MEDICATION_PROFILES[key] ? key : state.settings.medication;
}
function getInjectionsForMedication(key: string = state.settings.medication) {
    return (Array.isArray(state.injections) ? state.injections : [])
        .filter((i) => i && i.date && getInjectionMedicationKey(i) === key);
}
let activeTab = 'dashboard';
let weightChartInstance: ChartInstanceLike | null = null;
let symptomTrendChartInstance: ChartInstanceLike | null = null;
let pkCurveChartInstance: ChartInstanceLike | null = null;
let pkSelectedPointIndex: number | null = null;
let pkCurrentPointIndex: number = -1;
let selectedImageBase64: string | null = null;
let pendingAiBilingualResult: PendingAiBilingualResult | null = null;
let pacingTimerInterval: ReturnType<typeof setInterval> | null = null;
...
function cleanupTombstones() {
    ...
    (['meals', 'weights', 'injections', 'measurements', 'bowelMovements'] as const).forEach((type) => {
...
function initData() {
    ...
    try {
        ...
        if (raw) {
            ...
            if (Array.isArray(parsed.aiHistory))
                state.aiHistory = sanitizeAiHistory(parsed.aiHistory);
...
function bindStateToAuthenticatedUser(uid: string) {
    ensureStateMetadata();
...
function isPassiveEmptyDay(day: RuntimeDayData | null | undefined) {
...
function sanitizeCloudForCompare(value: unknown): unknown {
...
let localSafetyBackupCache: LocalSafetyBackupEntry[] = [];
...
function openLocalBackupDb(): Promise<IDBDatabase> {
    return new Promise((resolve, reject) => {
        ...
        req.onupgradeneeded = () => { const db = req.result; if (!db.objectStoreNames.contains('backups'))
            db.createObjectStore('backups', { keyPath: 'slot' }); if (!db.objectStoreNames.contains('emergency'))
            db.createObjectStore('emergency', { keyPath: 'key' }); };
        req.onsuccess = () => resolve(req.result);
        req.onerror = () => reject(req.error || new Error('INDEXEDDB_OPEN_FAILED'));
    });
}
async function idbPut(storeName: string, value: unknown) { const db = await openLocalBackupDb(); return new Promise((resolve, reject) => { const tx = db.transaction(storeName, 'readwrite'); tx.objectStore(storeName).put(value); tx.oncomplete = () => { db.close(); resolve(true); }; tx.onerror = () => { const e = tx.error; db.close(); reject(e || new Error('INDEXEDDB_WRITE_FAILED')); }; }); }
async function idbGetAll<T = unknown>(storeName: string): Promise<T[]> { const db = await openLocalBackupDb(); return new Promise((resolve, reject) => { const tx = db.transaction(storeName, 'readonly'); const req = tx.objectStore(storeName).getAll(); req.onsuccess = () => resolve(req.result || []); req.onerror = () => reject(req.error || new Error('INDEXEDDB_READ_FAILED')); tx.oncomplete = () => db.close(); }); }
async function idbClear(storeName: string) { const db = await openLocalBackupDb(); return new Promise((resolve, reject) => { const tx = db.transaction(storeName, 'readwrite'); tx.objectStore(storeName).clear(); tx.oncomplete = () => { db.close(); resolve(true); }; tx.onerror = () => { const e = tx.error; db.close(); reject(e || new Error('INDEXEDDB_CLEAR_FAILED')); }; }); }
async function initLocalSafetyBackups() {
    try {
        ...
        localSafetyBackupCache = (await idbGetAll<LocalSafetyBackupEntry>('backups')).filter((x) => Boolean(x && x.snapshot && typeof x.snapshot === 'object')).sort((a, b) => (Number(b.savedAt) || 0) - (Number(a.savedAt) || 0));
    }
    catch (err) {
...
function commitLocalSafetySnapshot(previous: RuntimeState | RuntimeCloudDocument | null | undefined) {
    try {
        if (!previous || typeof previous !== 'object')
            return;
        const existing = getLocalSafetyBackups();
        const previousComparable = stableStringify(previous);
        if (existing.some((x) => { try {
...
async function replaceLocalSafetyBackups(items: unknown) {
    const clean: LocalSafetyBackupEntry[] = (Array.isArray(items) ? items : []).slice(0, LOCAL_SAFETY_BACKUP_SLOTS).map((x, idx) => { const obj = (x && typeof x === 'object' && !Array.isArray(x)) ? x as UnknownRecord : {}; const snapshot = (obj.snapshot && typeof obj.snapshot === 'object' && !Array.isArray(obj.snapshot)) ? obj.snapshot as RuntimeState : createDefaultState(); return { slot: idx, savedAt: Number(obj.savedAt) || Date.now() + idx, appVersion: String(obj.appVersion || APP_VERSION), snapshot }; });
    try {
...
function isQuotaExceededError(err: unknown) { const e = (err && typeof err === 'object') ? err as { name?: unknown; code?: unknown } : {}; return Boolean(e.name === 'QuotaExceededError' || e.code === 22 || e.code === 1014); }
function emergencyStateToIndexedDb(snapshot: RuntimeState) { return idbPut('emergency', { key: 'latest', savedAt: Date.now(), appVersion: APP_VERSION, snapshot: JSON.parse(JSON.stringify(snapshot)) }).catch((err) => console.error('Emergency IndexedDB save failed:', err)); }
async function recoverEmergencyStateIfNewer() {
    try {
        const rows = await idbGetAll<EmergencyStateEntry>('emergency');
...
function persistState(options: { userMutation?: boolean; skipCloudSync?: boolean; localOnly?: boolean; immediateCloud?: boolean; suppressSafetyBackup?: boolean; skipLocalSafetyBackup?: boolean } = {}) {
    cleanupTombstones();
    ensureStateMetadata();
    ensureV4State();
    if (currentUser)
...
function scheduleCloudSync(immediate: boolean = false) {
...
function mergeSnapshotForTransaction(localSnapshot: RuntimeState, cloud: RuntimeCloudDocument): RuntimeCloudDocument {
...
function syncMergedStateToCloudIfNeeded(incomingCloud: RuntimeCloudDocument) {
...
function applyCloudDataMerge(cloud: RuntimeCloudDocument, options: { skipCloudWrite?: boolean; preferCloudOnTie?: boolean; suppressRender?: boolean; suppressLocalSave?: boolean } = {}) {
    ...
    const cloudMetaRaw: Partial<RuntimeMeta> = (cloud.meta && typeof cloud.meta === 'object' && !Array.isArray(cloud.meta)) ? cloud.meta : {};
    const cloudMeta = {
        ...
        settingsFieldUpdatedAt: (cloudMetaRaw.settingsFieldUpdatedAt && typeof cloudMetaRaw.settingsFieldUpdatedAt === 'object' && !Array.isArray(cloudMetaRaw.settingsFieldUpdatedAt)) ? cloudMetaRaw.settingsFieldUpdatedAt as StringMap<number> : {},
    ...
    if (cloudMeta.dataEpoch !== state.meta.dataEpoch) {
        ...
        if (cloudResetAt > localResetAt || (cloudResetAt === localResetAt && cloudMeta.dataEpoch !== 'legacy-v1')) {
            ...
            const replacementCloudSettings: Record<string, unknown> = (cloud.settings && typeof cloud.settings === 'object' && !Array.isArray(cloud.settings)) ? Object.assign({}, cloud.settings) as unknown as Record<string, unknown> : {};
            delete replacementCloudSettings.language;
            delete replacementCloudSettings.units;
            replacement.settings = Object.assign(replacement.settings, replacementCloudSettings) as RuntimeSettings;
            ...
            replacement.aiHistory = sanitizeAiHistory(cloud.aiHistory);
    ...
    const cloudSettings: Record<string, unknown> = (cloud.settings && typeof cloud.settings === 'object' && !Array.isArray(cloud.settings)) ? Object.assign({}, cloud.settings) as unknown as Record<string, unknown> : {};
    ...
    const fieldStamp = (map: StringMap<number>, hasMap: boolean, globalTs: number, key: string, parentKey: string | null = null) => {
    ...
    const mergeNestedSetting = (key: 'inventoryByProduct' | 'inventoryBaseByProduct' | 'roomTempTrackers') => {
        ...
        const localObj: Record<string, unknown> = (localValue && typeof localValue === 'object' && !Array.isArray(localValue)) ? localValue as Record<string, unknown> : {};
        const cloudObj: Record<string, unknown> = (cloudValue && typeof cloudValue === 'object' && !Array.isArray(cloudValue)) ? cloudValue as Record<string, unknown> : {};
        const out: Record<string, unknown> = JSON.parse(JSON.stringify(localObj)) as Record<string, unknown>;
        new Set([...Object.keys(localObj), ...Object.keys(cloudObj)]).forEach((sub) => { const stampKey = `${key}.${sub}`, lts = fieldStamp(localFieldTs, localHasFieldMap, localSettingsTs, stampKey, key), cts = fieldStamp(cloudFieldTs, cloudHasFieldMap, cloudSettingsTs, stampKey, key); if (sub in cloudObj && (cts > lts || (cts === lts && cts === 0 && options.preferCloudOnTie !== false && !(sub in localObj)))) {
    ...
    Object.keys(cloudSettings).forEach((key) => {
        ...
        if (cloudWins) {
            (state.settings as unknown as Record<string, unknown>)[key] = JSON.parse(JSON.stringify(cloudSettings[key]));
            if (cts > 0)
    ...
    const inventoryEventMap = new Map<string, RuntimeInventoryEvent>();
    ...
    if (cloud.daysData && typeof cloud.daysData === 'object' && !Array.isArray(cloud.daysData)) {
        Object.keys(cloud.daysData).forEach((dateKey) => {
            ...
            if (!rawCloudDay || typeof rawCloudDay !== 'object' || Array.isArray(rawCloudDay))
                return;
            const cloudDay = JSON.parse(JSON.stringify(rawCloudDay)) as RuntimeDayData;
            const cloudMeals: RuntimeMeal[] = Array.isArray(cloudDay.meals) ? cloudDay.meals : [];
            const cloudBowelMovements: string[] = Array.isArray(cloudDay.bowelMovements) ? cloudDay.bowelMovements : [];
            if (!state.daysData[dateKey] || typeof state.daysData[dateKey] !== 'object' || Array.isArray(state.daysData[dateKey])) {
                ...
                const mealMap = new Map<string, RuntimeMeal>();
                ...
                const waterMap = new Map<string, RuntimeWaterEvent>();
                [...(cloudDay.waterEvents || []), ...(localDay.waterEvents || [])].forEach((e) => { if (!e || !e.id)
                    return; const prev = waterMap.get(e.id); if (!prev || (Number(e.updatedAt) || 0) >= (Number(prev.updatedAt) || 0))
                ...
                const localActivityTs = Number(localDay.activityUpdatedAt) || 0;
                const cloudActivityTs = Number(cloudDay.activityUpdatedAt) || 0;
                if (cloudActivityTs > localActivityTs) {
                    localDay.activityMinutes = Math.max(0, Number(cloudDay.activityMinutes) || 0);
                    localDay.activityUpdatedAt = cloudActivityTs;
                }
                // v4.0.6 bowel movements: records have unique IDs and deletion tombstones.
                const bmMap = new Map<string, RuntimeBowelMovementRecord>();
                [...(cloudDay.bowelMovementRecords || []), ...(localDay.bowelMovementRecords || [])].forEach((r) => { if (!r || !r.id)
                    return; const del = Number(state.deletedRecords.bowelMovements?.[r.id]) || 0; if (del > (Number(r.updatedAt) || 0))
                    return; const prev = bmMap.get(r.id); if (!prev || (Number(r.updatedAt) || 0) >= (Number(prev.updatedAt) || 0))
                    bmMap.set(r.id, r); });
    ...
    if (Array.isArray(cloud.weights)) {
        const weightMap = new Map<string, RuntimeWeight>();
...
function replaceStateFromCloudCanonical(cloud: RuntimeCloudDocument) {
    if (!cloud || typeof cloud !== 'object' || Array.isArray(cloud))
        return;
    try {
