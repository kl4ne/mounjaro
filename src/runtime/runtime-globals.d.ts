...
interface PersonalProgressAiRequest {
  contextJson: string;
  modelName?: string | null;
}
...
interface Window {
  ...
  firebaseAiPersonalProgress?: (request: PersonalProgressAiRequest) => Promise<Record<string, unknown>>;
...
interface RuntimeSettings {
  ...
  weightGoalConfigured: boolean;
  proteinGoalConfigured: boolean;
  waterGoalConfigured: boolean;
  activityGoalConfigured: boolean;
  pensInStock: number;
...
interface RuntimeAiDeviceStats {
  ...
  events: RuntimeAiUsageEvent[];
  updatedAt: number;
}
type RuntimeAiUsageEventKind = 'action' | 'cache-hit' | 'generation-success' | 'generation-failure' | 'translation';
interface RuntimeAiUsageEvent {
  id: string;
  kind: RuntimeAiUsageEventKind;
  timestamp: number;
}
...
type RuntimeAiReportType = 'ask-data' | 'weekly-checkin' | 'pattern-finder' | 'visit-prep' | 'progress-comparison' | 'personal-progress';
