...
/* ==========================================================================
   ...
   ========================================================================== */
function getLatestWeight(): number | null { const arr = (state.weights || []).filter((w) => w && Number.isFinite(Number(w.weight))).sort((a, b) => String(a.date).localeCompare(String(b.date))); const latest = arr.at(-1); return latest ? Number(latest.weight) : null; }
function getWeightVariation(days: number = 30): number | null { const cutoff = getDateOffsetKey(getFormattedDate(new Date()), -(days - 1)) || ''; const vals = (state.weights || []).filter((w) => w && String(w.date) >= cutoff && Number.isFinite(Number(w.weight))).map((w) => Number(w.weight)); if (vals.length < 2)
    return null; return Math.max(...vals) - Math.min(...vals); }
function toggleMaintenanceMode() { const input = document.getElementById('maintenance-range-input'); const shown = Number(input && input.value); const range = storageWeight(shown); if (range !== null && Number.isFinite(range) && range >= 0.5 && range <= 15)
    state.settings.maintenanceRangeLb = range; state.settings.maintenanceEnabled = !state.settings.maintenanceEnabled; markSettingsChanged(['maintenanceRangeLb', 'maintenanceEnabled']); persistState({ userMutation: true }); renderMaintenanceMode(); showToast(state.settings.maintenanceEnabled ? uiText('Modo mantenimiento activado.', 'Maintenance mode enabled.') : uiText('Modo mantenimiento desactivado.', 'Maintenance mode disabled.')); }
function renderMaintenanceMode() {
    const badge = document.getElementById('maintenance-status-badge'), summary = document.getElementById('maintenance-summary'), explanation = document.getElementById('maintenance-explanation'), rangeEl = document.getElementById('maintenance-range'), curEl = document.getElementById('maintenance-current'), varEl = document.getElementById('maintenance-variation'), btn = document.getElementById('maintenance-toggle-btn'), input = document.getElementById('maintenance-range-input');
    if (!badge || !summary)
        return;
    ...
    if (explanation) explanation.innerText = uiText('Usa el Modo mantenimiento cuando quieras observar si tu peso permanece dentro del rango objetivo seleccionado. Al activarlo, GLP-1 Companion compara tu peso actual con tu peso objetivo ± el margen de mantenimiento y resume la estabilidad reciente. No cambia tu medicamento, dosis, meta de peso, comportamiento de IA, tratamiento ni cálculos PK.', 'Use Maintenance Mode when you want to monitor whether your weight stays within your selected target range. When enabled, GLP-1 Companion compares your current weight with your target weight ± the maintenance margin and summarizes recent stability. It does not change your medication, dose, weight goal, AI behavior, treatment, or PK calculations.');
    if (input) {
...
function calculateAgeFromDob(dob: unknown): number | null {
...
function saveHealthProfile() {
    ...
    const normalizedSex: RuntimeHealthProfile['sex'] = sex === 'female' || sex === 'male' ? sex : '';
    const normalizedActivity: RuntimeHealthProfile['activity'] = (['sedentary', 'light', 'moderate', 'active', 'very_active'] as string[]).includes(activity) ? activity as RuntimeHealthProfile['activity'] : '';
...
function renderHealthProfile() {
    ...
    const set = (id: string, val: string | number | null | undefined) => { const el = document.getElementById(id); if (el && el !== active)
...
function calculateSmartBodyMetrics(): SmartBodyMetrics {
    ...
    const out: SmartBodyMetrics = { bmi: null, bmr: null, tdee: null, whtr: null, bodyFat: null, fatMassLb: null, leanMassLb: null, hydrationOz: null };
    if (kg && cm)
        out.bmi = kg / Math.pow(cm / 100, 2);
    if (kg && cm && h.sex && age !== null) {
        out.bmr = 10 * kg + 6.25 * cm - 5 * age + (h.sex === 'male' ? 5 : -161);
        const mult = ({ sedentary: 1.2, light: 1.375, moderate: 1.55, active: 1.725, very_active: 1.9 } as Record<string, number>)[String(h.activity)];
        if (mult)
...
function renderSmartBodyMetrics() {
    const x = calculateSmartBodyMetrics(), set = (id: string, val: string | number) => { const el = document.getElementById(id); if (el)
...
function applySuggestedTdee() { const x = calculateSmartBodyMetrics(), el = document.getElementById('setting-tdee'); if (!el || x.tdee === null) {
    ...
    return;
} el.value = String(Math.round(x.tdee)); showToast(uiText('TDEE sugerido colocado. Pulsa Guardar configuración para aplicarlo.', 'Suggested TDEE filled in. Tap Save settings to apply it.')); }
function applySmartHydrationGoal() { const x = calculateSmartBodyMetrics(), el = document.getElementById('setting-water-goal'); if (!el || x.hydrationOz === null) {
    showToast(uiText('Registra un peso válido primero.', 'Log a valid weight first.'));
    return;
} el.value = String(Math.round(displayWater(x.hydrationOz)!)); showToast(uiText('Meta de hidratación sugerida colocada. Pulsa Guardar configuración para aplicarla.', 'Suggested hydration goal filled in. Tap Save settings to apply it.')); }
function openMetricInfo(key: string) {
    ...
    if (!modal || !title || !body)
        return;
    const content: Record<string, { es: readonly [string, string]; en: readonly [string, string] }> = {
...
function closeMetricInfo() { const modal = document.getElementById('modal-metric-info'); if (modal) {
    modal.classList.add('hidden');
    modal.classList.remove('flex');
} }
function bodyMetricBmiClassification(bmi: unknown, lang: 'en' | 'es' = isEnglish() ? 'en' : 'es') {
...
function roundedCanvasRect(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number, r: number, fill: string, stroke: string | null = null, lineWidth: number = 1) {
...
function canvasText(ctx: CanvasRenderingContext2D, text: unknown, x: number, y: number, size: number = 28, color: string = '#0f172a', weight: string = '400', align: CanvasTextAlign = 'left', font: string = 'Arial') { ctx.font = `${weight} ${size}px ${font}`; ctx.fillStyle = color; ctx.textAlign = align; ctx.textBaseline = 'alphabetic'; ctx.fillText(String(text ?? ''), x, y); }
function canvasWrap(ctx: CanvasRenderingContext2D, text: unknown, x: number, y: number, maxWidth: number, lineHeight: number, size: number = 24, color: string = '#475569', weight: string = '400', maxLines: number = 5): number {
...
function infographicPanel(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number, title: string, accent: string = '#1d4ed8') {
...
function infographicMetric(ctx: CanvasRenderingContext2D, x: number, y: number, label: string, value: string | number, accent: string = '#2563eb', sub: string = '') {
...
function drawBodyWeightChart(ctx: CanvasRenderingContext2D, data: ReturnType<typeof getBodyMetricsInfographicData>, x: number, y: number, w: number, h: number) {
    ...
    const px = (i: number) => weights.length === 1 ? x + w / 2 : x + (w * i / (weights.length - 1));
...
function drawEnergyDonut(ctx: CanvasRenderingContext2D, data: ReturnType<typeof getBodyMetricsInfographicData>, cx: number, cy: number, r: number) {
...
function infographicIcon(ctx: CanvasRenderingContext2D, type: string, cx: number, cy: number, size: number, color: string = '#1d4ed8') {
    ctx.save();
    ctx.translate(cx, cy);
    ...
    if (type === 'person') {
        ...
        ([[-.22, -.18], [0, -.24], [.22, -.12], [-.15, .04], [.10, .02], [.28, .15], [-.25, .23], [0, .25]] as Array<[number, number]>).forEach(([x, y]) => { ctx.beginPath(); ctx.arc(x * s, y * s, s * .055, 0, Math.PI * 2); ctx.fill(); });
    }
    else if (type === 'tape') {
        ctx.beginPath();
...
function infographicSoftPanel(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number, title: string, iconType: string | null = null, accent: string = '#1d4ed8') {
    ctx.save();
...
function infographicInfoLine(ctx: CanvasRenderingContext2D, iconType: string, label: string, value: string | number, x: number, y: number, color: string = '#12345b', sub: string = '') {
...
function infographicCompositionCard(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number, iconType: string, label: string, value: string | number, color: string = '#2563eb', sub: string = '') {
...
function drawEditorialWeightChart(ctx: CanvasRenderingContext2D, data: ReturnType<typeof getBodyMetricsInfographicData>, x: number, y: number, w: number, h: number) {
    const weights = data.weights;
    if (!weights.length) {
    ...
    if (max - min < 10)
        max = min + 10;
    const px = (i: number) => weights.length === 1 ? x + w * .14 : x + (w * i / (weights.length - 1));
    const py = (v: number) => y + h - ((v - min) / (max - min)) * h;
...
function drawEditorialEnergyDonut(ctx: CanvasRenderingContext2D, data: ReturnType<typeof getBodyMetricsInfographicData>, cx: number, cy: number, r: number) {
    const bmr = Number(data.metrics.bmr), tdee = Number(data.metrics.tdee);
    if (!Number.isFinite(bmr) || !Number.isFinite(tdee) || tdee <= 0) {
...
function renderBodyMetricsInfographicCanvas() {
    ...
    const cdata: Array<[string, string, number | null]> = [['Start', 'Inicio', d.startLb], ['Current', 'Actual', d.currentLb], ['Goal', 'Meta', d.goalLb]];
...
function closeBodyMetricsInfographic() { const modal = document.getElementById('modal-body-infographic'); if (modal) {
    modal.classList.add('hidden');
    modal.classList.remove('flex');
} document.body.classList.remove('body-infographic-print-mode'); }
function dataUrlToBytes(dataUrl: string): Uint8Array { const base64 = String(dataUrl).split(',')[1] || ''; const bin = atob(base64), out = new Uint8Array(bin.length); for (let i = 0; i < bin.length; i++)
    out[i] = bin.charCodeAt(i); return out; }
function buildJpegImagePdfBytes(jpegBytes: Uint8Array, imgW: number, imgH: number): Uint8Array {
    type PdfObject = string | { dict: string; bytes: Uint8Array };
    const pageW = 792, pageH = 612, objects: Array<PdfObject | undefined> = [];
    ...
    const parts: Uint8Array[] = [], offsets: number[] = [0];
    const push = (b: Uint8Array | string) => parts.push(b instanceof Uint8Array ? b : cp1252Bytes(b));
    push('%PDF-1.4\n%GLP1\n');
...
function removeWeightEntry(targetDate: string) {
    if (!targetDate)
        return;
    ...
    weightChartInstance = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: { callbacks: { label: (ctx: { dataset: { label?: string }; parsed: { y: unknown } }) => ` ${ctx.dataset.label}: ${ctx.parsed.y} ${weightUnit()}` } }
            },
            scales: {
                x: { grid: { display: false }, ticks: { color: '#64748b' } },
                y: { grid: { color: 'rgba(255,255,255,0.05)' }, ticks: { color: '#64748b', callback: (v: unknown) => `${v}` } }
            }
        }
    });
}
...
function saveMeasurementEntry() {
    ...
    const [waist, chest, hips, neck] = values as [number, number, number, number];
...
function playGentleChime(freq: number = 587.33) {
    try {
...
let pacingTargetEndTime: number | null = null;
function toggleMealPacingTimer() {
    ...
    pacingTimerInterval = setInterval(() => {
        ...
        if (pacingSecondsLeft <= 0) {
            if (pacingTimerInterval !== null) clearInterval(pacingTimerInterval);
            pacingTimerInterval = null;
            pacingTargetEndTime = null;
            pacingSecondsLeft = 20 * 60;
...
let fastingTimerInterval: ReturnType<typeof setInterval> | null = null;
...
function getAppointmentDateBounds(rangeValue: string): { startKey: string | null; endKey: string } {
    const end = new Date();
...
function buildAppointmentSummary(rangeValue: string, reportLang: 'es' | 'en' | null = null): string {
    const lang = reportLang || state.settings.language || 'es';
    const en = lang === 'en';
    const tx = (es: string, enText: string): string => en ? enText : es;
    const bounds = getAppointmentDateBounds(rangeValue);
    const inRange = (dateKey: string | null | undefined): boolean => Boolean(dateKey && (!bounds.startKey || dateKey >= bounds.startKey) && dateKey <= bounds.endKey);
...
function refreshAppointmentSummary() {
    ...
    const selectedLang: 'es' | 'en' = (langEl ? langEl.value : state.settings.language) === 'en' ? 'en' : 'es';
...
function copyToClipboardText(text: string) {
...
function saveNutritionalSettings() {
    ...
    if (goalWeight === null || waterGoal === null) return;
    ...
    state.settings.weightGoalConfigured = true;
    state.settings.proteinGoalConfigured = true;
    state.settings.waterGoalConfigured = true;
    ...
    markSettingsChanged(['goalWeight', 'calGoal', 'tdee', 'proteinGoal', 'waterGoal', 'weightGoalConfigured', 'proteinGoalConfigured', 'waterGoalConfigured', 'maintenanceRangeLb']);
    persistState({ userMutation: true });
    updateDashboardView();
    renderToolsView();
    applyUnitLabels();
...
function formatBackupFileStamp(date: Date = new Date()): string {
...
function sanitizedBackupState(snapshot: unknown): UnknownRecord {
    const copy = JSON.parse(JSON.stringify(snapshot || {})) as UnknownRecord;
    if (copy.settings && typeof copy.settings === 'object' && !Array.isArray(copy.settings))
        delete (copy.settings as UnknownRecord).apiKey;
    return copy;
}
function createCurrentBackupObject(snapshot: unknown = state): UnknownRecord {
...
async function deliverJsonBackup(exportObject: unknown, filename: string): Promise<void> {
...
function downloadBackupSnapshot(snapshot: unknown, label: string = '') {
...
function renderExportBackupModalLanguage() {
    const set = (id: string, es: string, en: string) => { const el = document.getElementById(id); if (el)
...
function closeExportBackupOptions() { const modal = document.getElementById('modal-export-backup'); if (modal) {
    modal.classList.add('hidden');
    modal.classList.remove('flex');
} }
function firestoreTimestampToMillis(value: unknown): number {
    ...
    if (typeof value === 'object' && !Array.isArray(value)) {
        ...
        if (typeof record.toMillis === 'function')
            return Number((record.toMillis as () => unknown)()) || 0;
        if (typeof record.toDate === 'function') {
            const d = (record.toDate as () => unknown)();
...
let serverSafetyBackupsCache: ServerSafetyBackupEntry[] = [];
async function fetchServerSafetyBackups(): Promise<ServerSafetyBackupEntry[]> {
    if (!currentUser)
        throw new Error('AUTH_REQUIRED');
    if (navigator.onLine === false)
        throw new Error('OFFLINE');
    const snap = await firebasePlatform.getSafetyDocs(currentUser.uid);
    const items: ServerSafetyBackupEntry[] = [];
    snap.forEach((doc) => {
        const raw = doc.data();
        if (!raw || typeof raw !== 'object' || Array.isArray(raw)) return;
        const d = raw as UnknownRecord;
        if (!d.snapshot || typeof d.snapshot !== 'object' || Array.isArray(d.snapshot)) return;
        items.push({
            slot: doc.id,
            revision: Number(d.revision) || 0,
            savedAt: firestoreTimestampToMillis(d.savedAt),
            snapshot: d.snapshot as RuntimeState | RuntimeCloudDocument
        });
    });
...
function backupSummary(snapshot: unknown): BackupCountSummary {
    const validation = validateBackupStructure(snapshot);
    if (validation.ok)
        return validation.counts;
    const source: UnknownRecord = snapshot && typeof snapshot === 'object' && !Array.isArray(snapshot) ? snapshot as UnknownRecord : {};
    let meals = 0, days = 0;
    const daysData = source.daysData && typeof source.daysData === 'object' && !Array.isArray(source.daysData) ? source.daysData as UnknownRecord : {};
    Object.values(daysData).forEach((day) => {
        if (!day || typeof day !== 'object' || Array.isArray(day)) return;
        const record = day as UnknownRecord;
        days++;
        if (Array.isArray(record.meals)) meals += record.meals.length;
    });
    return {
...
function closeServerSafetyBackups() { const m = document.getElementById('modal-server-backups'); if (m) {
    m.classList.add('hidden');
    m.classList.remove('flex');
} }
function closeBackupView() { const m = document.getElementById('modal-backup-view'); if (m) {
    m.classList.add('hidden');
    m.classList.remove('flex');
} }
function viewServerSafetyBackup(slot: string) {
...
function restoreServerSafetyBackup(slot: string) {
    ...
    showImportPreview(item.snapshot as unknown as UnknownRecord, Object.assign({}, validation, { version: `${validation.version} · Server Safety` }));
}
async function exportBackupPackage(mode: 'current' | 'local' | 'server' | 'complete' = 'current') {
    try {
...
function normalizeBackupEnvelope(raw: unknown): BackupEnvelope {
    if (isUnknownRecord(raw) && raw.backupPackage) {
        ...
        const localBackups: unknown[] = Array.isArray(raw.localBackups) ? raw.localBackups.slice(0, LOCAL_SAFETY_BACKUP_SLOTS) : [];
        const serverBackups: unknown[] = Array.isArray(raw.serverBackups) ? raw.serverBackups.slice(0, SERVER_SAFETY_BACKUP_SLOTS) : [];
        return {
...
function normalizeImportedTimestamp(value: unknown): number {
...
function normalizeImportedActivityFields(raw: UnknownRecord): { activityMinutes: number; activityUpdatedAt: number } {
    return { activityMinutes: Math.max(0, Math.min(1440, Number(raw.activityMinutes) || 0)), activityUpdatedAt: Number(raw.activityUpdatedAt) > 0 ? normalizeImportedTimestamp(raw.activityUpdatedAt) : 0 };
}
let pendingImportCandidate: UnknownRecord | null = null;
let pendingImportPackageInfo: BackupPackageInfo | null = null;
function validateBackupStructure(imported: unknown): BackupStructureValidation {
    ...
    for (const key of ['injections', 'weights', 'measurements', 'aiHistory']) {
    ...
    let legacy = false;
    let backupMeta: UnknownRecord | null = null;
    ...
    const counts: BackupCountSummary = {
    ...
    if (isUnknownRecord(imported.daysData)) {
        Object.values(imported.daysData).forEach((day) => {
            if (!isUnknownRecord(day)) return;
            counts.days++;
            if (Array.isArray(day.meals)) counts.meals += day.meals.length;
        });
    }
...
function showImportPreview(imported: UnknownRecord, validation: BackupValidationSuccess) {
...
function cancelImportPreview() { pendingImportCandidate = null; pendingImportPackageInfo = null; const modal = document.getElementById('modal-import-preview'); if (modal) {
    modal.classList.add('hidden');
    modal.classList.remove('flex');
} }
...
async function applyImportedBackup(imported: UnknownRecord, packageInfo: BackupPackageInfo | null = null): Promise<void> {
    ...
    const localLanguage: 'es' | 'en' = state.settings.language === 'en' ? 'en' : 'es';
    const fallbackUnits: 'imperial' | 'metric' = state.settings.units === 'metric' ? 'metric' : 'imperial';
    const next = createDefaultState({ language: localLanguage, units: fallbackUnits });
    if (isUnknownRecord(imported.settings)) {
        const incoming: UnknownRecord = Object.assign({}, imported.settings);
        delete incoming.apiKey;
        delete incoming.language;
        delete incoming.units;
        next.settings = Object.assign(next.settings, incoming as Partial<RuntimeSettings>, { language: localLanguage, units: fallbackUnits });
        const importedMeta = isUnknownRecord(imported.meta) ? imported.meta : {};
        const importedFieldTimes = isUnknownRecord(importedMeta.settingsFieldUpdatedAt) ? importedMeta.settingsFieldUpdatedAt : {};
        const migratedGoals: Array<[keyof RuntimeSettings, keyof RuntimeSettings]> = [['weightGoalConfigured', 'goalWeight'], ['proteinGoalConfigured', 'proteinGoal'], ['waterGoalConfigured', 'waterGoal']];
        migratedGoals.forEach(([flag, value]) => {
            (next.settings as unknown as Record<string, unknown>)[flag] = resolveGoalConfigured(incoming, importedFieldTimes, String(flag), String(value));
        });
    }
    ...
    if (isUnknownRecord(imported.daysData)) {
        Object.keys(imported.daysData).forEach((dateKey) => {
            ...
            if (!isUnknownRecord(rawValue)) return;
            const raw = rawValue;
            const meals: RuntimeMeal[] = [];
            if (Array.isArray(raw.meals)) {
                raw.meals.forEach((mealValue: unknown, idx: number) => {
                    if (!isUnknownRecord(mealValue)) return;
            ...
            const waterEvents: RuntimeWaterEvent[] = Array.isArray(raw.waterEvents) ? raw.waterEvents.flatMap((entry: unknown) => {
                if (!isUnknownRecord(entry) || typeof entry.id !== 'string') return [];
                return [{ id: entry.id, delta: Number(entry.delta) || 0, updatedAt: normalizeImportedTimestamp(entry.updatedAt) }];
            }) : [];
            const bowelMovements: string[] = Array.isArray(raw.bowelMovements) ? raw.bowelMovements.filter((value: unknown): value is string => typeof value === 'string' && !Number.isNaN(new Date(value).getTime())) : [];
            const bowelMovementRecords: RuntimeBowelMovementRecord[] = Array.isArray(raw.bowelMovementRecords) ? raw.bowelMovementRecords.flatMap((entry: unknown) => {
                if (!isUnknownRecord(entry) || typeof entry.id !== 'string' || typeof entry.at !== 'string' || Number.isNaN(new Date(entry.at).getTime())) return [];
                return [{ id: entry.id, at: entry.at, updatedAt: normalizeImportedTimestamp(entry.updatedAt) }];
            }) : [];
            const defaultSymptoms: RuntimeSymptoms = { hunger: 3, nausea: 'none', heartburn: 'none', sulfur: 'none', fatigue: 'none', notes: '' };
            const symptoms: RuntimeSymptoms = isUnknownRecord(raw.symptoms) ? raw.symptoms as RuntimeSymptoms : defaultSymptoms;
            const electrolytes: Record<string, boolean> = isUnknownRecord(raw.electrolytes) ? raw.electrolytes as Record<string, boolean> : {};
            const water = parseFloat(String(raw.water ?? ''));
            const importedActivity = normalizeImportedActivityFields(raw);
            const day: RuntimeDayData = {
                water: Number.isFinite(water) && water >= 0 ? water : 0,
                waterUpdatedAt: normalizeImportedTimestamp(raw.waterUpdatedAt),
                waterEvents,
                activityMinutes: importedActivity.activityMinutes,
                activityUpdatedAt: importedActivity.activityUpdatedAt,
                meals,
                bowelMovements,
    ...
    if (Array.isArray(imported.weights)) {
        imported.weights.forEach((weightValue: unknown) => {
            if (!isUnknownRecord(weightValue)) return;
    ...
    if (Array.isArray(imported.injections)) {
        imported.injections.forEach((injectionValue: unknown) => {
            if (!isUnknownRecord(injectionValue)) return;
            const date = normalizeDateKey(injectionValue.date);
            if (!date || typeof injectionValue.dose !== 'string') return;
    ...
    if (Array.isArray(imported.measurements)) {
        imported.measurements.forEach((measurementValue: unknown) => {
            if (!isUnknownRecord(measurementValue)) return;
    ...
    next.aiHistory = sanitizeAiHistory(imported.aiHistory);
    ...
    const settingsFieldUpdatedAt: StringMap<number> = {};
...
function importDataJSON(e: Event) {
    const input = e.target as AppElement | null;
    ...
    reader.onload = function (evt: ProgressEvent<FileReader>) {
        try {
            const raw: unknown = JSON.parse(String(evt.target?.result || ''));
...
function showToast(msg: string, duration: number = 3500, preserveUndo: boolean = false) {
...
function handleDayRollover(forceToday: boolean = false) {
