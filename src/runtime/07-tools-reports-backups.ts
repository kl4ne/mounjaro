    ...
    if (isUnknownRecord(imported.daysData)) {
        Object.keys(imported.daysData).forEach((dateKey) => {
            ...
            const day: RuntimeDayData = {
                ...
                waterEvents,
                activityMinutes: Math.max(0, Math.min(1440, Number(raw.activityMinutes) || 0)),
                activityUpdatedAt: Number(raw.activityUpdatedAt) > 0 ? normalizeImportedTimestamp(raw.activityUpdatedAt) : 0,
                meals,
                bowelMovements,
