...
const CLIENT_SCHEMA_VERSION = 15;
...
function setSyncStatus(mode: 'syncing' | 'waiting' | 'update-required' | 'error' | 'synced' | 'local', timestamp: number | null = null) {
...
let serverClockAnchorMs: number | null = null;
let serverClockPerfAnchor: number | null = null;
...
function clearExplicitMutationPending(completedRevision?: number) {
...
function parseSemVer(v: unknown) {
    const parts = String(v || '').replace(/^[^\d]*/, '').split('.').map((n) => parseInt(n, 10) || 0);
    while (parts.length < 3)
        parts.push(0);
    return parts;
}
function compareSemVer(v1: unknown, v2: unknown) {
...
function repairFutureMutationTimestamps(serverMs: number) {
    ...
    const fix = (v: unknown) => { const n = Number(v) || 0; if (n > ceiling) {
        changed = true;
        seq += 1;
    ...
    Object.values(state.daysData || {}).forEach((day) => {
        ...
        (day.meals || []).forEach((x) => { if (x)
            x.updatedAt = fix(x.updatedAt); });
        (day.waterEvents || []).forEach((x) => { if (x)
            x.updatedAt = fix(x.updatedAt); });
        (day.bowelMovementRecords || []).forEach((x) => { if (x)
            x.updatedAt = fix(x.updatedAt); });
    });
    (state.injections || []).forEach((x) => { if (x)
        x.updatedAt = fix(x.updatedAt); });
    (state.inventoryEvents || []).forEach((x) => { if (x)
        x.updatedAt = fix(x.updatedAt); });
    (state.weights || []).forEach((x) => { if (x)
        x.updatedAt = fix(x.updatedAt); });
    (state.measurements || []).forEach((x) => { if (x)
        x.updatedAt = fix(x.updatedAt); });
    Object.values(state.deletedRecords || {}).forEach((bucket) => { if (!bucket || typeof bucket !== 'object')
        return; Object.keys(bucket).forEach((k) => { bucket[k] = fix(bucket[k]); }); });
    Object.values(state.settings?.roomTempTrackers || {}).forEach((t) => { if (t && typeof t === 'object')
    ...
    Object.values(state.aiTelemetry || {}).forEach((month) => {
        ...
        Object.values(month.devices || {}).forEach((device) => { device.updatedAt = fix(device.updatedAt); (['primary', 'fallback'] as const).forEach((role) => { const st = device[role]; if (!st)
            return; st.lastSuccessAt = fix(st.lastSuccessAt); st.lastFailureAt = fix(st.lastFailureAt); st.lastEventAt = fix(st.lastEventAt); }); });
    });
    if (changed) {
...
function updateServerClockAnchor(cloud: RuntimeCloudDocument | null | undefined) {
    try {
        const ts = cloud && cloud._sync && cloud._sync.serverUpdatedAt;
        const tsRecord = ts && typeof ts === 'object' ? ts as { toMillis?: () => number; seconds?: number } : null;
...
function generateDataEpoch(prefix: string = 'epoch') {
    try {
...
function estimateUtf8Bytes(value: unknown) {
    try {
...
function renderHardeningUiLanguage() {
    const en = isEnglish();
    const set = (id: string, text: string) => { const el = document.getElementById(id); if (el)
        el.textContent = text; };
...
function setPrivacyLock(uid: string | null | undefined) {
    try {
...
function runtimeErrorMessage(error: unknown): string {
    ...
    if (error && typeof error === 'object' && !Array.isArray(error)) {
        const value = (error as UnknownRecord).message;
...
function runtimeErrorCode(error: unknown): string {
    if (error && typeof error === 'object' && !Array.isArray(error)) {
        const value = (error as UnknownRecord).code;
...
function runtimeErrorName(error: unknown): string {
    ...
    if (error && typeof error === 'object' && !Array.isArray(error)) {
        const value = (error as UnknownRecord).name;
        return typeof value === 'string' ? value : '';
    }
    return '';
}
...
const STATIC_TRANSLATIONS: Record<string, string> = {
    ...
    "Acciones AI": "AI actions",
    "Llamadas API": "API calls",
    "Smart Cache": "Smart Cache",
    "API evitadas": "API calls avoided",
    "Eficiencia estimada de Smart Cache": "Estimated Smart Cache efficiency",
    "Las acciones AI cuentan lo que solicitas; las llamadas API cuentan intentos reales a los modelos. Smart Cache puede completar una acción sin consumir una llamada. Esta app no conoce ni inventa el porcentaje restante de la cuota de Google.": "AI actions count what you request; API calls count real model attempts. Smart Cache can complete an action without consuming an API call. This app does not know or invent the percentage of Google quota remaining.",
...
const STATIC_ATTRIBUTE_TRANSLATIONS: Record<string, string> = {
    "Ej: 180": "e.g. 180",
...
function uiText(es: string, en: string) {
    return isEnglish() ? en : es;
}
function translateSite(site: unknown) {
    if (!site)
    ...
    const pair = sites[String(site) as keyof typeof sites];
    return pair ? (isEnglish() ? pair[1] : pair[0]) : String(site);
}
function translateRuntimePhrase(text: string) {
    if (!text)
        return text;
...
function setAppLanguage(lang: 'es' | 'en' | string, shouldPersist: boolean = true) {
...
function displayWeight(lb: unknown) { const n = Number(lb); return Number.isFinite(n) ? (isMetric() ? n * LB_TO_KG : n) : null; }
function storageWeight(displayValue: unknown) { const n = Number(displayValue); return Number.isFinite(n) ? (isMetric() ? n / LB_TO_KG : n) : null; }
function displayWater(oz: unknown) { const n = Number(oz); return Number.isFinite(n) ? (isMetric() ? n * OZ_TO_ML : n) : null; }
function storageWater(displayValue: unknown) { const n = Number(displayValue); return Number.isFinite(n) ? (isMetric() ? n / OZ_TO_ML : n) : null; }
function displayLength(inches: unknown) { const n = Number(inches); return Number.isFinite(n) ? (isMetric() ? n * IN_TO_CM : n) : null; }
function storageLength(displayValue: unknown) { const n = Number(displayValue); return Number.isFinite(n) ? (isMetric() ? n / IN_TO_CM : n) : null; }
function formatWeight(lb: unknown, decimals: number = 1) { const n = displayWeight(lb); return n === null ? '--' : `${n.toFixed(decimals)} ${weightUnit()}`; }
function formatWater(oz: unknown, decimals: number | null = null) { const n = displayWater(oz); if (n === null)
    return '--'; const d = decimals === null ? (isMetric() ? 0 : 0) : decimals; return `${n.toFixed(d)} ${waterUnit()}`; }
function formatLength(inches: unknown, decimals: number = 1) { const n = displayLength(inches); return n === null ? '--' : `${n.toFixed(decimals)} ${lengthUnit()}`; }
function setUnitSystem(value: 'imperial' | 'metric' | string, shouldPersist: boolean = true) {
...
function applyUnitLabels() {
    ...
    ['meas-waist', 'meas-chest', 'meas-hips', 'meas-neck'].forEach((id) => { const el = document.getElementById(id); if (el) {
        el.min = metric ? '13' : '5';
