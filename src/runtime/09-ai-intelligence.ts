...
interface AiDataDailySnapshot {
    ...
    doses: Array<{ medication: string; dose: string; time: string }>;
}

interface AiDataCoverage {
    rangeDays: number;
    trackedDays: number;
    waterDays: number;
    mealDays: number;
    mealEntries: number;
    symptomDays: number;
    bowelMovementEntries: number;
    doseEntries: number;
    weightEntries: number;
}

interface AiWeekMetrics {
    startDate: string;
    endDate: string;
    trackedDays: number;
    symptomDays: number;
...
interface AiDataContext {
    ...
    displayUnits: { weight: string; water: string };
    activeMedication: { name: string; route: string; presentation: string };
    goals: { water: number; waterUnit: string; proteinG: number; calories: number; goalWeight: number; weightUnit: string };
    summary: {
        avgWater: number | null;
        avgProteinG: number | null;
    ...
    currentWeek: AiWeekMetrics;
    previousWeek: AiWeekMetrics | null;
    coverage: AiDataCoverage;
    weights: Array<{ date: string; value: number; unit: string }>;
    daily: AiDataDailySnapshot[];
}

interface AskMyDataResult {
    answerEs: string;
    answerEn: string;
    evidenceEs: string;
    evidenceEn: string;
...
let lastAskMyDataResult: AskMyDataResult | null = null;
let lastWeeklyAiResult: WeeklyAiResult | null = null;
let aiIntelligenceBusy = false;

function roundAiMetric(value: number | null, decimals: number = 1): number | null {
    if (value === null || !Number.isFinite(value)) return null;
    const factor = Math.pow(10, decimals);
    return Math.round(value * factor) / factor;
}

function truncateAiText(value: unknown, maxLength: number = 80): string {
    const text = String(value || '').trim().replace(/\s+/g, ' ');
    if (text.length <= maxLength) return text;
    return `${text.slice(0, Math.max(0, maxLength - 1)).trim()}…`;
}

function getAiBowelCount(day: RuntimeDayData): number {
    if (Array.isArray(day.bowelMovementRecords) && day.bowelMovementRecords.length) return day.bowelMovementRecords.length;
    if (Array.isArray(day.bowelMovements)) return day.bowelMovements.length;
    return 0;
}

function getAiSymptomSnapshot(day: RuntimeDayData): Record<string, string | number> {
    const symptoms = day.symptoms || {};
    const out: Record<string, string | number> = {};
    const symptomKeys: Array<keyof RuntimeSymptoms> = ['nausea', 'heartburn', 'sulfur', 'fatigue'];
    ...
    if (Number.isFinite(hunger)) out.hunger = hunger;
    return out;
}

function getAiDoseEntriesForDate(dateKey: string): Array<{ medication: string; dose: string; time: string }> {
...
function buildAiDailySnapshot(dateKey: string): AiDataDailySnapshot {
    ...
    const totals = meals.reduce<{ calories: number; protein: number; carbs: number; fat: number }>((acc, meal) => {
    ...
    return {
        ...
        meals: meals.slice(0, 8).map((meal) => truncateAiText(meal.nameEs || meal.name || meal.nameEn, 72)).filter(Boolean),
        symptoms,
...
function toAiWeekMetrics(metrics: ReturnType<typeof getWeekMetrics>): AiWeekMetrics {
...
function buildAiDataContext(rangeDays: number): AiDataContext {
    ...
    daily.forEach((day) => {
        ...
        if (Object.keys(day.symptoms).some((key) => key !== 'hunger')) symptomDays++;
...
function normalizeAskMyDataResult(data: Record<string, unknown>): AskMyDataResult {
    return {
...
function normalizeWeeklyAiResult(data: Record<string, unknown>): WeeklyAiResult {
    return {
...
function renderAiEvidenceLines(targetId: string, value: string): void {
...
function renderAskMyDataResult(): void {
...
function renderWeeklyAiResult(): void {
...
function renderWeeklyDeterministicPreview(): void {
...
function updateAiIntelligenceLanguage(): void {
    ...
    if (typeof updateAiPhase2Language === 'function') updateAiPhase2Language();
}

function renderAiIntelligenceView(): void {
    updateAiIntelligenceLanguage();
    renderWeeklyDeterministicPreview();
    renderAskMyDataResult();
    renderWeeklyAiResult();
    if (typeof renderAiPhase2View === 'function') renderAiPhase2View();
    if (typeof renderAiHistoryView === 'function') renderAiHistoryView();
    lucide.createIcons();
}

function setAskMyDataPreset(preset: string): void {
...
async function runAiIntelligenceRequest(
    request: (modelName: string) => Promise<Record<string, unknown>>,
    busyLabelId: string
): Promise<Record<string, unknown>> {
    if (aiIntelligenceBusy) throw new Error('AI_INTELLIGENCE_BUSY');
    if (navigator.onLine === false) throw new Error('AI_INTELLIGENCE_OFFLINE');
    if (window.firebaseAiLogicReady === false) throw new Error('AI_INTELLIGENCE_UNAVAILABLE');
    ...
    try {
        ...
        try {
            ...
            recordAiGenerationOutcome(true);
            ...
            telemetryChanged = true;
            if (!classification.recoverable) throw primaryError;
            recordAiFailover();
        }
        ...
        recordAiGenerationOutcome(false);
        telemetryChanged = true;
        throw error;
    } finally {
        if (telemetryChanged) {
...
function showAiIntelligenceError(error: unknown): void {
...
async function askMyData(): Promise<void> {
    ...
    const historyFingerprint = createAiHistoryFingerprint({
        type: 'ask-data',
        question: question.trim().replace(/\s+/g, ' ').toLocaleLowerCase(),
        context
    });
    if (tryUseAiHistoryCache('ask-data', historyFingerprint)) return;

    document.getElementById('ai-data-answer-card').classList.add('hidden');
    try {
        const output = await runAiIntelligenceRequest(
            (modelName) => window.firebaseAiAskMyData!({ question, contextJson: JSON.stringify(context), modelName }),
            'ask-data-button-text'
        );
...
async function generateWeeklyAiCheckIn(): Promise<void> {
    ...
    const historyFingerprint = createAiHistoryFingerprint({ type: 'weekly-checkin', context: weeklyContext });
    if (tryUseAiHistoryCache('weekly-checkin', historyFingerprint)) return;

    document.getElementById('weekly-ai-result-card').classList.add('hidden');
    try {
        const output = await runAiIntelligenceRequest(
            (modelName) => window.firebaseAiWeeklyCheckIn!({ contextJson: JSON.stringify(weeklyContext), modelName }),
            'weekly-ai-button-text'
        );
        ...
        saveAiHistoryReport({
            ...
            fingerprint: historyFingerprint,
            rangeDays: 7,
            result: lastWeeklyAiResult,
            context: Object.assign({ range: { startDate: context.currentWeek.startDate, endDate: context.currentWeek.endDate, days: 7 }, summary: context.summary }, weeklyContext)
        });
        renderWeeklyAiResult();
        lucide.createIcons();
    } catch (error) {
