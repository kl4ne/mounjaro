...
function handleImageSelected(e: Event): void {
    ...
    reader.onload = function (evt: ProgressEvent<FileReader>) {
        const rawBase64 = String(evt.target?.result || '');
        const img = new Image();
        img.onload = function () {
...
function classifyAiFailure(error: unknown): AiFailureClassification {
    const errObj = (error && typeof error === 'object') ? error as { message?: unknown; code?: unknown; status?: unknown } : {};
    const message = String(errObj.message || error || '').toLowerCase();
    const code = String(errObj.code || errObj.status || '').toLowerCase();
    const combined = `${code} ${message}`;
    if (typeof navigator !== 'undefined' && navigator.onLine === false)
        return { recoverable: false, reason: 'offline' };
    ...
    if (nonRecoverable.some((x) => combined.includes(x)))
        return { recoverable: false, reason: 'security' };
    const recoverable = [
    ...
    if (error instanceof SyntaxError || recoverable.some((x) => combined.includes(x)))
        return { recoverable: true, reason: 'temporary_or_invalid_response' };
...
Analyze only the food shown in the image and/or described by the user. Estimate calories and macronutrients from the actual meal details; never reuse fixed example values. Return the meal name naturally in BOTH Spanish (foodNameEs) and English (foodNameEn), plus a concise neutral GLP-1 tolerance-tracking note in BOTH Spanish (tipEs) and English (tipEn). The numeric calories/protein/carbs/fat values are shared across languages. Do not diagnose, do not recommend medication changes, and do not invent unsupported ingredients. If portion size is uncertain, use a reasonable small-to-medium homemade portion estimate and mention that uncertainty briefly in both tolerance notes.`;
    ...
    if (selectedImageBase64) {
        const pieces = selectedImageBase64.split(',');
        imageData = pieces[1] || null;
        mimeType = (pieces[0] || '').split(';')[0].replace('data:', '') || 'image/jpeg';
    }
...
function populateAiResults(data: Record<string, unknown>, sourceLabel: string): void {
    ...
    document.getElementById('res-cal').value = String(safeNumber(data?.calories));
    document.getElementById('res-protein').value = String(safeNumber(data?.protein));
    document.getElementById('res-carbs').value = String(safeNumber(data?.carbs));
    document.getElementById('res-fat').value = String(safeNumber(data?.fat));
    ...
    if (badgeText)
        badgeText.innerText = sourceLabel || uiText('Análisis listo (Ajustable)', 'Analysis ready (Editable)');
...
function commitScannedMeal() {
    ...
    if (isEnglish())
        pair.foodNameEn = visibleName;
    else
        pair.foodNameEs = visibleName;
    if (isEnglish())
        pair.tipEn = visibleTip;
    else
        pair.tipEs = visibleTip;
...
async function translateSavedMeal(idx: number): Promise<void> {
    const day = getSelectedDay(), meal = Array.isArray(day.meals) ? day.meals[Number(idx)] : null;
    if (!meal)
        return;
    if (meal.nameEs && meal.nameEn && meal.tipEs && meal.tipEn) {
        renderMealsList();
        return;
    }
    if (typeof navigator !== 'undefined' && navigator.onLine === false) {
        showToast(uiText('Conéctate a internet para crear la versión bilingüe una sola vez.', 'Connect to the internet to create the bilingual version once.'));
        return;
    }
    if (typeof window.firebaseAiTranslateMeal !== 'function' || window.firebaseAiLogicReady === false) {
        showToast(uiText('Firebase AI Logic no está disponible para traducir ahora.', 'Firebase AI Logic is not available for translation right now.'));
        return;
    }
    ...
    if (!sourceTip) {
        showToast(uiText('Este registro no tiene análisis de IA para convertir.', 'This record has no AI analysis to convert.'));
        return;
    }
    recordAiUserAction();
    ...
    const run = async (model: string, role: 'primary' | 'fallback'): Promise<Record<string, unknown>> => { const t0 = performance.now(); try {
        const translateMeal = window.firebaseAiTranslateMeal!;
        const out = await translateMeal({ foodName: sourceName, tip: sourceTip, modelName: model });
        recordAiAttempt(role, true, performance.now() - t0);
        return out;
    }
    catch (err) {
        const c = classifyAiFailure(err);
        if (c.recoverable)
            recordAiAttempt(role, false, performance.now() - t0);
        const wrapped = err instanceof Error ? err : new Error(String(err));
        (wrapped as Error & { _classification?: AiFailureClassification })._classification = c;
        throw wrapped;
    } };
    try {
        try {
            translated = await run(AI_PRIMARY_MODEL, 'primary');
        }
        catch (err) {
            const c = (err instanceof Error ? (err as Error & { _classification?: AiFailureClassification })._classification : undefined) || classifyAiFailure(err);
            if (!c.recoverable)
                throw err;
            recordAiFailover();
            translated = await run(AI_FALLBACK_MODEL, 'fallback');
        }
        meal.nameEs = String(translated?.foodNameEs || sourceName).trim();
        meal.nameEn = String(translated?.foodNameEn || sourceName).trim();
        meal.tipEs = String(translated?.tipEs || sourceTip).trim();
        meal.tipEn = String(translated?.tipEn || sourceTip).trim();
        meal.aiGenerated = true;
        meal.analysisLanguage = 'bilingual';
        meal.updatedAt = mutationNow();
        recordAiTranslation();
        recordAiGenerationOutcome(true);
        persistState({ userMutation: true });
        renderMealsList();
        renderAiStatus();
        showToast(uiText('Versión bilingüe guardada. No será necesario volver a generarla.', 'Bilingual version saved. It will not need to be generated again.'));
    }
    catch (err) {
        recordAiGenerationOutcome(false);
        console.warn('Saved meal bilingual conversion failed', err);
        persistState({ userMutation: true });
        renderAiStatus();
        showToast(uiText('No se pudo crear la versión bilingüe ahora.', 'The bilingual version could not be created right now.'), 5000);
    }
}
