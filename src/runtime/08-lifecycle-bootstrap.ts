...
function startGlp1Application() {
    ...
    document.getElementById('hub-injections-list').addEventListener('click', (e) => {
        const btn = (e.target as Element).closest('[data-delete-inj]') as HTMLElement | null;
        if (btn) {
            e.stopPropagation();
            removeInjection(btn.dataset.deleteInj || '');
            return;
        }
        const row = (e.target as Element).closest('[data-open-inj]') as HTMLElement | null;
        if (row)
            openInjectionDetailModal(row.dataset.openInj || '');
    });
    document.getElementById('weights-history-list').addEventListener('click', (e) => {
        const btn = (e.target as Element).closest('[data-delete-weight]') as HTMLElement | null;
        if (btn) {
            removeWeightEntry(btn.dataset.deleteWeight || '');
        }
    });
    document.getElementById('daily-meals-container').addEventListener('click', (e) => {
        const translateBtn = (e.target as Element).closest('[data-translate-meal]') as HTMLElement | null;
        if (translateBtn) {
            translateSavedMeal(parseInt(translateBtn.dataset.translateMeal || '0', 10));
            return;
        }
        const btn = (e.target as Element).closest('[data-delete-meal]') as HTMLElement | null;
        if (btn)
