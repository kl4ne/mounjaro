...
/* ==========================================================================
   ...
   ========================================================================== */
const MEDICATION_PROFILES: Record<string, MedicationProfile> = {
...
const MEDICATION_PRESENTATIONS: Readonly<Record<string, readonly MedicationPresentation[]>> = Object.freeze({
    mounjaro: [
...
function getPresentationOptions(medKey: string = state?.settings?.medication || 'mounjaro'): readonly MedicationPresentation[] { return MEDICATION_PRESENTATIONS[medKey] || [{ key: 'default', es: 'Estándar', en: 'Standard', timerMode: 'none', inventoryMode: 'manual' }]; }
function getDefaultPresentationKey(medKey: string = state?.settings?.medication || 'mounjaro'): string { return getPresentationOptions(medKey)[0].key; }
function getPresentationProfile(medKey: string = state?.settings?.medication || 'mounjaro', presentationKey: string | undefined = state?.settings?.presentation): MedicationPresentation { const opts = getPresentationOptions(medKey); return opts.find((x) => x.key === presentationKey) || opts[0]; }
function getProductKey(medKey: string = state?.settings?.medication || 'mounjaro', presentationKey: string | undefined = state?.settings?.presentation): string { return `${medKey}:${presentationKey || getDefaultPresentationKey(medKey)}`; }
function getCurrentProductKey() { return getProductKey(state.settings.medication, state.settings.presentation); }
function sanitizeInventoryEvents(input: unknown): RuntimeInventoryEvent[] {
    if (!Array.isArray(input))
        return [];
    const byId = new Map<string, RuntimeInventoryEvent>();
...
function inventoryEventTotal(productKey: string): number {
    return sanitizeInventoryEvents(state.inventoryEvents).reduce((sum, e) => e.productKey === productKey ? sum + e.delta : sum, 0);
}
function recalcInventoryCount(productKey: string = getCurrentProductKey()): number {
...
function ensureInventoryLedgerState(productKey: string = getCurrentProductKey()): void {
    if (!state.settings.inventoryByProduct || typeof state.settings.inventoryByProduct !== 'object' || Array.isArray(state.settings.inventoryByProduct))
        state.settings.inventoryByProduct = {};
    if (!state.settings.inventoryBaseByProduct || typeof state.settings.inventoryBaseByProduct !== 'object' || Array.isArray(state.settings.inventoryBaseByProduct))
        state.settings.inventoryBaseByProduct = {};
...
function getInventoryCount(productKey: string = getCurrentProductKey()): number { ensureInventoryLedgerState(productKey); return recalcInventoryCount(productKey); }
function setInventoryCount(value: unknown, productKey: string = getCurrentProductKey()): number {
...
function upsertInventoryEvent(id: unknown, delta: unknown, productKey: string = getCurrentProductKey(), reason: string = 'manual', sourceId: string = ''): number {
    ensureInventoryLedgerState(productKey);
...
function recordInventoryDelta(delta: unknown, productKey: string = getCurrentProductKey(), reason: string = 'manual', sourceId: string = ''): number {
    ensureInventoryLedgerState(productKey);
...
function getDoseInventoryEventId(injectionId: unknown): string { return `dose:${String(injectionId || '')}`; }
function getLegacyReturnInventoryEventId(injectionId: unknown): string { return `legacy-return:${String(injectionId || '')}`; }
function hasInventoryEvent(id: string): boolean { return Array.isArray(state.inventoryEvents) && state.inventoryEvents.some((e) => e && e.id === id); }
function consumeInventoryForDose(injectionId: string, productKey: string): number {
    return upsertInventoryEvent(getDoseInventoryEventId(injectionId), -1, productKey, 'dose', injectionId);
}
function returnInventoryForDeletedDose(injectionId: string, productKey: string): number {
...
function reverseInventoryReturnForUndo(injectionId: string, productKey: string): number {
    const doseId = getDoseInventoryEventId(injectionId);
    if (hasInventoryEvent(doseId))
...
function getRoomTempTracker(productKey: string = getCurrentProductKey()): RuntimeRoomTempTracker { if (!state.settings.roomTempTrackers || typeof state.settings.roomTempTrackers !== 'object')
    state.settings.roomTempTrackers = {}; if (!state.settings.roomTempTrackers[productKey])
    state.settings.roomTempTrackers[productKey] = { active: false, startedAt: 0, accumulatedMs: 0, firstUseAt: 0, updatedAt: 0 }; return state.settings.roomTempTrackers[productKey]; }
function simpleStableHash(text: unknown): string { let h = 2166136261; for (let i = 0; i < String(text).length; i++) {
    h ^= String(text).charCodeAt(i);
    h = Math.imul(h, 16777619);
} return (h >>> 0).toString(36); }
function ensureGranularDayData(dateKey: string, day: RuntimeDayData): RuntimeDayData {
    day.activityMinutes = Math.max(0, Math.min(1440, Number(day.activityMinutes) || 0));
    day.activityUpdatedAt = Math.max(0, Number(day.activityUpdatedAt) || 0);
    ...
    const seen = new Set();
    day.bowelMovementRecords = day.bowelMovementRecords.filter((r) => { if (!r || typeof r.id !== 'string' || seen.has(r.id) || typeof r.at !== 'string' || isNaN(new Date(r.at).getTime()))
        return false; seen.add(r.id); return true; }).map((r) => ({ id: r.id, at: r.at, updatedAt: Math.max(0, Number(r.updatedAt) || 0) }));
...
function recalcWaterFromEvents(day: RuntimeDayData | null | undefined): number { if (!day)
    return 0; const total = (Array.isArray(day.waterEvents) ? day.waterEvents : []).reduce((sum, e) => sum + Number(e?.delta || 0), 0); day.water = Math.max(0, total); day.waterUpdatedAt = Math.max(Number(day.waterUpdatedAt) || 0, ...(day.waterEvents || []).map((e) => Number(e?.updatedAt) || 0), 0); return day.water; }
function getMedicationProfile(key: string | null = null): MedicationProfile {
    return MEDICATION_PROFILES[key || state.settings.medication] || MEDICATION_PROFILES.mounjaro;
}
function getMedicationLabel(profile: MedicationProfile | null = null, includeGeneric: boolean = false): string {
    const p = profile || getMedicationProfile();
    return includeGeneric ? `${p.name} (${p.generic})` : p.name;
}
function getRegimenLabel(profile: MedicationProfile | null = null): string {
    const p = profile || getMedicationProfile();
    if (p.route === 'oral')
...
/* ==========================================================================
   ...
   ========================================================================== */
function handleMedicationChange(selectedMed: string): void {
...
function handlePresentationChange(value: string): void { const opts = getPresentationOptions(); const next = opts.some((x) => x.key === value) ? value : opts[0].key; if (next === state.settings.presentation)
...
function storageTrackerElapsedRoomMs(tracker: RuntimeRoomTempTracker, now: number = Date.now()): number { return Math.max(0, Number(tracker.accumulatedMs) || 0) + (tracker.active && Number(tracker.startedAt) > 0 ? Math.max(0, now - Number(tracker.startedAt)) : 0); }
function getStorageTrackerStatus(now: number = Date.now()) {
    const pr = getPresentationProfile(), t = getRoomTempTracker(), dayMs = 86400000;
    let roomLeft: number | null = null, firstUseLeft: number | null = null;
    if (pr.roomTempDays)
    ...
    const values = [roomLeft, firstUseLeft].filter((v): v is number => typeof v === 'number' && Number.isFinite(v));
...
function noteFirstUseForCurrentProduct(atMs: number = Date.now()): void {
    const pr = getPresentationProfile();
    if (!pr.firstUseDays)
        return;
...
function applyMedicationTheme() {
    ...
    [siteWrap, hubSiteRow, zones].forEach((el) => { if (el)
...
function cp1252Bytes(value: unknown): Uint8Array {
...
function pdfEscapeLiteral(value: unknown): string {
    return normalizePdfText(value).replace(/\\/g, '\\\\').replace(/\(/g, '\\(').replace(/\)/g, '\\)');
}
function wrapPdfLine(line: unknown, maxChars: number = 88): string[] {
...
function buildTextPdfBytes(title: string, bodyText: string, language: string): Uint8Array {
    const W = 612, H = 792, margin = 48, topY = 738, bottomY = 48;
    const wrapped: Array<{ text: string; heading: boolean }> = [];
    ...
    const pages: Array<Array<{ text: string; heading: boolean; y: number }>> = [];
    let page: Array<{ text: string; heading: boolean; y: number }> = [], y = topY - 58;
    ...
    if (page.length || !pages.length)
        pages.push(page);
    const objects: Array<string | { stream: string; length: number } | undefined> = [];
    ...
    const pushBytes = (b: Uint8Array | string) => byteParts.push(b instanceof Uint8Array ? b : cp1252Bytes(b));
    pushBytes('%PDF-1.4\n%GLP1\n');
    const offsets = [0];
...
async function presentPdfOnApple(title: string, bodyText: string, language: string, filenameBase: string): Promise<boolean> {
    try {
...
function getSelectedDay() {
    if (!state.daysData[state.selectedDate]) {
        state.daysData[state.selectedDate] = {
            water: 0, waterUpdatedAt: 0, waterEvents: [], activityMinutes: 0, activityUpdatedAt: 0, meals: [],
