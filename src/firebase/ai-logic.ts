...
try {
  ...
  window.firebaseAiPersonalProgress = async ({ contextJson, modelName }: PersonalProgressAiRequest): Promise<JsonRecord> => {
    ...
    const model = getGenerativeModel(aiService, {
      model: selectedModel,
      systemInstruction: 'Use only the supplied calculated data. Do not recalculate source data. Do not invent missing values. Do not invent goals or create medical target values. Missing data remains missing. Respect the 4-of-7 coverage rule and acknowledge limited coverage. Mention only explicitly recorded symptoms. Distinguish an observed pattern or correlation from certainty. Do not diagnose. Do not claim causation. Do not claim treatment effectiveness. Do not recommend increasing medication, decreasing medication, skipping medication, stopping medication, or switching medication. Do not prescribe a treatment plan. Do not define a medically appropriate rate of weight loss. Explain the supplied trends cautiously in concise, supportive language. When appropriate, suggest discussing questions with a healthcare professional calmly and without alarmist language. Return equivalent natural Spanish and English. observations and limitations use concise bullet-style lines separated by newline characters; discussion is an optional neutral topic the user may discuss with a healthcare professional.',
