...
try {
  ...
  window.firebaseAiAnalyzeMeal = async ({ systemPrompt, prompt, imageBase64, mimeType, modelName }: MealAiRequest) => {
  ...
  window.firebaseAiTranslateMeal = async ({ foodName, tip, modelName }: MealTranslationRequest) => {
    const selectedModel = modelName === AI_MODELS.fallback ? AI_MODELS.fallback : AI_MODELS.primary;
    const responseSchema = {
      type: 'object',
      properties: {
  ...
  window.firebaseAiLogicInitError = error instanceof Error ? error.message : String(error || 'Unknown AI initialization error');
}
