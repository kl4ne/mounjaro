...
function hasUnsafeEditingState(): boolean {
  /*
   * Any visible app modal may contain unsaved input.
   * Never force a reload there.
   */
  ...
  /*
   * Outside a modal, protect a field only
   * after the user has interacted during
   * this page session.
   *
   * Browser-restored focus alone must not
   * block startup updates.
   */
  if (firstInteractionAt) {
    const active =
...
function handleWaitingWorker(
  ...
  /*
   * Only an actively unsafe editing
   * state gets a deferred-update notice.
   */
  showDeferredBanner();
...
async function registerServiceWorker():
  ...
  try {
    ...
    /*
     * Force a startup check without
     * relying on browser HTTP cache.
     */
...
export function startPwaUpdateManager():
  ...
  /*
   * Public bridge used by the
   * legacy-compatible runtime while
   * v5 is migrated.
   */
  window.glp1Pwa = {
    applyUpdateNow:
