/**
 * GLP-1 Companion v5.0.2 PWA update manager.
 *
 * Goals:
...
function canActivateAutomatically() {
  ...
  if (isFreshLaunchWindow()) return true;
...
function refreshBannerLanguage() {
  ...
  if (title) title.textContent = languageIsEnglish ? 'Update paused' : 'Actualización en espera';
  if (text) {
  ...
  if (button) button.textContent = languageIsEnglish ? 'Update now' : 'Actualizar ahora';
}
...
interface WorkerBuildInfo { type?: string; buildId?: string; }

async function getWorkerBuildInfo(worker: ServiceWorker | null): Promise<WorkerBuildInfo | null> {
  if (!worker) return null;
  return new Promise<WorkerBuildInfo | null>((resolve) => {
...
interface ActivateOptions { automatic?: boolean; reason?: string; }

async function activateWaitingWorker({ automatic = true, reason = 'safe' }: ActivateOptions = {}): Promise<boolean> {
  ...
  let fromVersion = '';
  try { fromVersion = localStorage.getItem(VERSION_SEEN_KEY) || localStorage.getItem(LEGACY_VERSION_SEEN_KEY) || ''; } catch (_) {}
...
function scheduleSafeRetry() {
  ...
  retryTimer = setInterval(() => {
    ...
    if (Date.now() - retryStartedAt > SAFE_RETRY_LIMIT_MS) clearRetryTimer();
  }, SAFE_RETRY_MS);
}

function deferUntilVisible(callback: () => void): void {
...
function handleWaitingWorker(nextRegistration?: ServiceWorkerRegistration | null): void {
...
function installControllerChangeHandler() {
  navigator.serviceWorker.addEventListener('controllerchange', () => {
    if (!reloadRequested || reloadInProgress) return;
...
export function startPwaUpdateManager(): void {
  ...
  window.glp1Pwa = {
    ...
    showCompletedUpdateToast: ({ version, isEnglish, showToast }: Glp1PwaUpdateToastArgs): boolean => {
      let pending = null;
      try { pending = JSON.parse(localStorage.getItem(UPDATE_RELOAD_KEY) || localStorage.getItem(LEGACY_UPDATE_RELOAD_KEY) || 'null'); } catch (_) {}
      try { localStorage.setItem(VERSION_SEEN_KEY, String(version || '')); } catch (_) {}
      if (!pending) return false;
      try { localStorage.removeItem(UPDATE_RELOAD_KEY); localStorage.removeItem(LEGACY_UPDATE_RELOAD_KEY); } catch (_) {}
