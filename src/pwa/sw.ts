...
async function networkFirst(
  request: Request,
  cacheName: string,
  fallbackRequest: RequestInfo | URL | null = null
): Promise<Response> {
...
sw.addEventListener('fetch', (event: FetchEvent) => {
  ...
  const staticDestination: readonly RequestDestination[] = ['script', 'style', 'font', 'image', 'manifest'];
  if (!staticDestination.includes(request.destination)) return;
  ...
  if (sameOrigin) {
    ...
    if ((['script', 'style', 'manifest'] as readonly RequestDestination[]).includes(request.destination)) {
