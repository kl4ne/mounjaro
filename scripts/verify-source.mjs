...
if (pkg.version !== '5.0.2') throw new Error(`Unexpected package version: ${pkg.version}`);
...
if (runtimeFiles.length !== 8) throw new Error(`Expected 8 TypeScript runtime modules, found ${runtimeFiles.length}`);

const sw = await readFile(join(root, 'src/pwa/sw.ts'), 'utf8');
if (!sw.includes("v5.0.2-pwa")) throw new Error('Service worker source build ID is not v5.0.2-pwa');
