...
// v5.5.0 keeps the proven 10-runtime architecture while making TypeScript the only runtime source.
...
if (pkg.version !== '5.5.0') throw new Error(`Unexpected package version: ${pkg.version}`);
...
if (manifest.version !== '5.5.0') throw new Error(`Unexpected manifest version: ${manifest.version}`);
...
for (const marker of [
  ...
  'v5.5.0 · Progressive Web App',
  'id="ai-user-actions"',
  'id="ai-api-calls"',
  'id="ai-cache-efficiency"',
  'id="ai-last-30-days"'
]) {
  if (!html.includes(marker)) throw new Error(`Missing v5.5.0 UI marker: ${marker}`);
}
...
for (const marker of [
  ...
  if (!core.includes(marker)) throw new Error(`Missing v5.5.0 core marker: ${marker}`);
}
...
for (const fn of [
  ...
  if (!phase2Runtime.includes(`function ${fn}`) && !phase2Runtime.includes(`async function ${fn}`)) {
    throw new Error(`Missing v5.5.0 runtime function in module 10: ${fn}`);
  }
}
for (const marker of [
...
if (!sw.includes("v5.5.0-pwa")) throw new Error('Service worker source build ID is not v5.5.0-pwa');
...
console.log(`Verified v5.5.0 source: ${srcFiles.length} src files, ${runtimeFiles.length} TypeScript runtime modules, no generated JavaScript checked in, AI Usage Analytics present, no duplicate IDs, all inline handlers resolved.`);
