...
function sanitizeAiHistory(input) {
    if (!Array.isArray(input))
        return [];
    const allowedTypes = new Set(['ask-data', 'weekly-checkin', 'pattern-finder', 'visit-prep', 'progress-comparison']);
    const out = [];
    input.forEach((raw) => {
        ...
        if (!/^[A-Za-z0-9_-]{6,120}$/.test(id) || !allowedTypes.has(type))
            return;
        const cleanResult = {};
        if (src.result && typeof src.result === 'object' && !Array.isArray(src.result)) {
            Object.entries(src.result).forEach(([key, value]) => {
        ...
        const snapshot = (src.snapshot && typeof src.snapshot === 'object' && !Array.isArray(src.snapshot))
            ? JSON.parse(JSON.stringify(src.snapshot))
            : {};
...
function mergeAiHistory(localValue, cloudValue) {
    const map = new Map();
