...
function getLastNDaysKeys(days = 30) {
    const end = new Date();
...
function calculatePersonalInsights(days = 30) {
    ...
    keys.forEach((k) => {
        const d = state.daysData[k];
        if (!d)
            return;
        if (dayHasMeaningfulTracking(d))
            tracked++;
        const s = d.symptoms || {};
        const symptomatic = !!((s.nausea && s.nausea !== 'none') || (s.heartburn && s.heartburn !== 'none') || (s.sulfur && s.sulfur !== 'none'));
        if (symptomatic)
            symptomDays++;
    ...
    getInjectionsForMedication().forEach((i) => {
        if (!i || !i.date)
            return;
        const base = String(i.date).split('T')[0];
        [0, 1, 2].forEach((o) => postDoseKeys.add(getDateOffsetKey(base, o)));
    });
    let postTracked = 0, postSym = 0, otherTracked = 0, otherSym = 0;
    keys.forEach((k) => {
        const d = state.daysData[k];
        if (!d || !dayHasMeaningfulTracking(d))
            return;
        const s = d.symptoms || {};
        const symptomatic = !!((s.nausea && s.nausea !== 'none') || (s.heartburn && s.heartburn !== 'none') || (s.sulfur && s.sulfur !== 'none'));
        if (postDoseKeys.has(k)) {
            postTracked++;
            if (symptomatic)
                postSym++;
        }
...
function renderPersonalInsights() {
    ...
    if (!el)
        return;
    const x = calculatePersonalInsights(30), items = [];
...
function updateHalfLifeAndNextDose() {
    ...
    if (isOral) {
        ...
        const rotations = { 'Abdomen derecho': 'Abdomen izquierdo', 'Abdomen izquierdo': 'Muslo derecho', 'Muslo derecho': 'Muslo izquierdo', 'Muslo izquierdo': 'Brazo derecho', 'Brazo derecho': 'Brazo izquierdo', 'Brazo izquierdo': 'Abdomen derecho' };
