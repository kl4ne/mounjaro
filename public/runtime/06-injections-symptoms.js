...
function adjustPenCount(delta) {
    const key = getCurrentProductKey();
    const next = recordInventoryDelta(delta, key, 'manual-adjustment');
    const p = getMedicationProfile();
    const el = document.getElementById('pen-stock-val');
    if (el)
        el.innerText = `${next} ${isEnglish() ? p.inventoryUnitEn : p.inventoryUnitEs}`;
    persistState({ userMutation: true });
    renderMounjaroView();
}
...
function getBowelStats() {
    ...
    Object.values(state.daysData || {}).forEach((day) => {
        (Array.isArray(day.bowelMovements) ? day.bowelMovements : []).forEach((ts) => {
...
function getPkRecords(profile = getMedicationProfile(), atTs = Date.now()) {
    return getInjectionsForMedication(profile.key)
        ...
        .filter((x) => typeof x.ts === 'number' && Number.isFinite(x.ts) && x.ts > 0 && typeof x.dose === 'number' && Number.isFinite(x.dose) && x.dose > 0 && x.ts <= atTs)
        .sort((a, b) => b.ts - a.ts);
}
...
function renderPkCurve(profile, est) {
    ...
    if (pkCurveChartInstance) {
        ...
        if (pkCurveChartInstance.options.scales?.y)
...
function removeInjection(id) {
    ...
    renderMounjaroView();
    updateDashboardView();
    showUndoToast(uiText('Registro de dosis eliminado.', 'Dose record deleted.'), () => {
        if (inventoryReturned && removedCopy.inventoryConsumed && removedCopy.productKey)
            reverseInventoryReturnForUndo(id, removedCopy.productKey);
        removedCopy.updatedAt = mutationNow();
        state.injections.push(removedCopy);
        state.injections.sort((a, b) => (injectionTimestamp(a) || 0) - (injectionTimestamp(b) || 0));
        if (state.deletedRecords?.injections)
            delete state.deletedRecords.injections[id];
        persistState({ userMutation: true });
        renderMounjaroView();
        updateDashboardView();
    });
...
function renderSymptomTrendChart() {
    ...
    symptomTrendChartInstance = new Chart(canvas.getContext('2d'), { type: 'line', data: { labels, datasets: [
            ...
            ] }, options: { responsive: true, maintainAspectRatio: false, interaction: { mode: 'index', intersect: false }, plugins: { legend: { labels: { color: '#94a3b8', boxWidth: 10, font: { size: 9 } } }, tooltip: { callbacks: { label: (ctx) => {
                            if (ctx.raw === null || ctx.raw === undefined)
                                return ` ${ctx.dataset.label}: ${uiText('Sin datos', 'No data')}`;
                            const words = isEnglish() ? ['None', 'Mild', 'Moderate', 'High'] : ['Ninguno', 'Leve', 'Moderado', 'Alto'];
                            return ` ${ctx.dataset.label}: ${words[Math.max(0, Math.min(3, ctx.parsed.y || 0))]}`;
                        } } } }, scales: { x: { grid: { display: false }, ticks: { color: '#64748b', maxTicksLimit: 8 } }, y: { min: 0, max: 3, ticks: { stepSize: 1, color: '#64748b', callback: (v) => isEnglish() ? ['None', 'Mild', 'Moderate', 'High'][Number(v)] : ['Ninguno', 'Leve', 'Moderado', 'Alto'][Number(v)] }, grid: { color: 'rgba(255,255,255,0.05)' } } } } });
}
